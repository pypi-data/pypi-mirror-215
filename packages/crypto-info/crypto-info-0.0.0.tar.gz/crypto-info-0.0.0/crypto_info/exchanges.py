...
def load_exchanges(
    ...
    return {
        exchange["id"]: Exchange(
            name=exchange["id"], title=exchange["name"], url=exchange["url"],
            logo=str(Path(f"{assets()}/logos/exchanges/{exchange['id']}.jpg")),
            trust=exchange["trust_score"], rank=exchange["trust_score_rank"],
            beginning=exchange["year_established"], country=exchange["country"],
            description=exchange["description"] or None
