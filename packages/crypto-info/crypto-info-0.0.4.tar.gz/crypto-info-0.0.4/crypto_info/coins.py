...
def load_coins_data(
    ...
    if reload or (not raw_coins):
        ...
        raw_coins.update(
            {coin['id']: coin for coin in table_to_json(df).values()}
        )
    ...
    if coins is not None:
        data = {key: coin for key, coin in data.items() if key in coins}
    # end if
...
def load_coins(
    ...
    if data is None:
        data = load_coins_data(coins=coins, reload=reload)
    # end if
