...
from git import Repo, HEAD, Diff, InvalidGitRepositoryError
import shutil
...
class CLIGitRelease:
    ...
    def validate_local_for_release(self, current_release: Dict[str, Any]):
        if self.is_dirty_to_release():
            raise CLIGitReleaseException(FeedbackManager.error_commit_changes_to_release())
        elif not self.is_head_outdated(current_release['commit']):
            raise CLIGitReleaseException(FeedbackManager.error_head_outdated(commit=self.head().commit.hexsha))
        elif not self.is_new_release(current_release['commit']):
