...
from catline.utils import catline_json as json
...
class QueueStorageJSONAdapter(IFQueueStorageAdapter):
    ...
    async def get_jobs(
        ...
        for priority in JOB_PRIORITIES:
            if len(result) == limit:
                break
            for key, job in self.queues["jobs"][f"priority_{priority}"].items():  # TODO: JSONの形式を変更する。 nameの中に優先度を作成することで、他のジョブをforから除外できる
                _status = job["status"]
                _name = job["name"]
