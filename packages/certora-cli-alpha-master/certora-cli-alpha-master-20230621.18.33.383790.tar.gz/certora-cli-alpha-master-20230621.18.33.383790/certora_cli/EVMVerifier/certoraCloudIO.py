...
def compare_cloud_versions_check(local_main: int, local_patch: int, local_sub: int, remote_main: int, remote_sub: int,
                                 remote_patch: int,
                                 package_name: str, recommended_upgrade_msg: str, major_upgrade_msg: str) -> None:
    """
    ...
    if package_name == PRODUCTION_PACKAGE_NAME:
        ...
        if remote_main != local_main or remote_sub != local_sub or remote_patch != local_patch:
            raise CertoraUserInputError(
                f"New beta version available {remote_main}.{remote_sub}.{remote_patch}, please upgrade." +
                f" (Current version is {local_main}.{local_sub}.{local_patch})")
