...
def get_package_data(exclude_pattern):
    matches = []
    for root, dirnames, filenames in os.walk('isoCycle'):
        for filename in fnmatch.filter(filenames, '*.*'):
            if not fnmatch.fnmatch(filename, exclude_pattern):
                # Relative path from the package root
                relpath = os.path.relpath(root, 'isoCycle')
                # Convert relative path to pattern
                pattern = os.path.join(relpath, filename)
                matches.append(pattern)
    return {'isoCycle': matches}
...
setup(
  ...
  package_data=get_package_data('*.pdf'),
  # package_data={
