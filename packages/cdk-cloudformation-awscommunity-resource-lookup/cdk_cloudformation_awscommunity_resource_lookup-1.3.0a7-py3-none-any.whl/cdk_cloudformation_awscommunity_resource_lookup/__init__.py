...
> AWS CDK [L1 construct](https://docs.aws.amazon.com/cdk/latest/guide/constructs.html) and data structures for the [AWS CloudFormation Registry](https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/registry.html) type `AwsCommunity::Resource::Lookup` v1.3.0.
...
class CfnLookupProps:
    def __init__(
        ...
        *,
        ...
        If only one match is found, this resource returns the primary identifier of the resource (in the ``AWS::EC2::VPC`` example, the ID of the VPC), that you can then reference in your template with the ``Fn::GetAtt`` intrinsic function.  Specify resource type search targets that are supported by Cloud Control API.
