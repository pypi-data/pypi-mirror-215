...
#tags that could have multiple <source> tags nested inside
AUDIO_VIDEO_TAGS = ['audio', 'video'] 

#tags that can be processed if they have an associated src
STANDARD_MULTI_MODAL_TAGS = ['img', 'video', 'audio', 'source'] 

#tags that can be processed if they have an associated src that's flagged as a multi-media provider
EMBEDDED_CONTENT_TAGS = ['iframe', 'embed'] 

#map html tags to output token tags
MULTI_MODAL_TAG_MAPPING = {'img':'IMAGE', 'audio':'AUDIO', 'video':'VIDEO','iframe':'VIDEO',
                    'embed':'VIDEO'} 
                    
#keywords to look for in src for embedded tags
...
def preprocessor(dom):
    ...
    options = {
        ...
        "comments": True,
        "style": True,
        "embedded": False, #include embedded content in parsed HTML
        "forms": True,
        "kill_tags": ("head",),
    }
...
class ParagraphMaker(ContentHandler):
    ...
    def __init__(self):
        ...
        self.link = False
        self.br = False
        self.src_found = False #used to keep track of <source> tags if they are nested in an <audio> or <video> tag
    ...
    def startElementNS(self, name, qname, attrs):
        ...
        #set source to not found if an open audio/video tag is found
        if name in ['audio', 'video']:
            self.src_found = False 

        src = attrs.get((None, 'src'), '')
        
        #process the tag if tag name could contain multi_modal content and has an associated source
	#OR if tag name is embedded content with multi-media provider in src URL
        if name in STANDARD_MULTI_MODAL_TAGS and src or (name in EMBEDDED_CONTENT_TAGS and any(provider in src for provider in MULTIMEDIA_PROVIDERS)):
            #if it's a <source> tag, and nested directly under an <audio> or <video> tag, change name and set src_found to True
            if name == 'source':
                if not self.src_found and self.path._elements[-2][0] in AUDIO_VIDEO_TAGS:
                    name = self.path._elements[-2][0]
                    self.src_found = True
                else:
                    return  #<source> tag is not relevant, return
            #create new multi-modal paragraph block with appropriate tag
            self._start_new_pragraph()
    ...
    for paragraph in paragraphs:
        ...
        if link_density > max_link_density:
            ...
            paragraph.cf_class = 'neargood'
                ...
                paragraph.cf_class = 'short' #set all multi-modal blocks to neutral designation of 'short'
        else:
            paragraph.cf_class = 'bad'
