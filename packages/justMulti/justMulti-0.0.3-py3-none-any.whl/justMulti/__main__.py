...
def main():
    import getopt
    from justMulti import __version__ as VERSION
    ...
    try:
        for o, a in opts:
            ...
            if o in ("-V", "--version"):
                ...
                if a.lower() == 'none':
                    ...
                    if os.path.isfile(a):
                        ...
                        if re.match(r'^\w*$', a):
                            ...
                            raise justMultiInvalidOptions("File not found: %s" % a)
            elif o == "--encoding":
                try:
                    default_encoding = a
                    ''.encode(default_encoding)
                except LookupError:
                    raise justMultiInvalidOptions("Uknown character encoding: %s" % a)
            elif o == "--enc-force":
                ...
                if a.lower() in ['strict', 'ignore', 'replace']:
                    ...
                    raise justMultiInvalidOptions("Invalid --enc-errors value: %s" % a)
            elif o == "--format":
                if a in ['default', 'boilerplate', 'detailed', 'krdwrd']:
                    format = a
                else:
                    raise justMultiInvalidOptions("Uknown output format: %s" % a)
            elif o == "--no-headings":
                no_headings = True
            elif o == "--length-low":
                try:
        ...
        if stoplist is None:
            raise justMultiInvalidOptions("No stoplist specified.")
        ...
        paragraphs = justMulti(html_text, stoplist, length_low, length_high,
