...
class SecurityCore:
    ...
    def acquire_access_token_with_refresh_token(cls, sp_client_id: str, sp_client_secret: str, sp_tenant_id: str,
        ...
        azure_databricks_resource_id = config_user["azure_databricks_resource_id"]
        print(f"creating context for az_sub_oauth_token_endpoint:{az_sub_oauth_token_endpoint}")
        context = AuthenticationContext(az_sub_oauth_token_endpoint)
        if context is None:
            print(f"AuthenticationContext: None : not acquired for az_sub_oauth_token_endpoint:{az_sub_oauth_token_endpoint}")
        elif len(str(context)) == 0:
            print(f"AuthenticationContext: empty string : not acquired for az_sub_oauth_token_endpoint:{az_sub_oauth_token_endpoint}")
        else:
            print(f"AuthenticationContext acquired for az_sub_oauth_token_endpoint:{az_sub_oauth_token_endpoint}")
    ...
    def setup_user_configuration(sp_client_id: str, sp_client_secret: str, sp_tenant_id: str, sp_redirect_url: str,
        ...
        sp_az_sub_oauth_token_endpoint = "".join([sp_authority_host_url.rstrip("/"), "/", sp_tenant_id])
    ...
    @staticmethod
    def verify_az_sub_client_secret(tenant_id, client_id, client_secret):
