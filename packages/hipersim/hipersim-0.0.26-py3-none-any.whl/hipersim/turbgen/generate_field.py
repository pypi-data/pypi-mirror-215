...
class SpectralTensor():
    ...
    def generate_field(self, seed, alphaepsilon=None):
        ...
        n = (rg.standard_normal((N1, N2, N3), dtype='float32') +
             rg.standard_normal((N1, N2, N3), dtype='float32') * 1j)
        Cxyz = SqrtPhi[:, 0] * n
        n = (rg.standard_normal((N1, N2, N3), dtype='float32') +
             rg.standard_normal((N1, N2, N3), dtype='float32') * 1j)
        Cxyz += SqrtPhi[:, 1] * n
        ...
        scale = np.sqrt(alphaepsilon / self.alphaepsilon)
        u = np.real(u[:Nx, :Ny, :Nz]) * scale
        v = np.real(v[:Nx, :Ny, :Nz]) * scale
        w = np.real(w[:Nx, :Ny, :Nz]) * scale
        return u, v, w
