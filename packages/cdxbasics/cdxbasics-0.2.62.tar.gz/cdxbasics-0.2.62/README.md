...
* `quantile(P,x,quantiles,axis)` computes quantiles of `x` using the distribution `P`. If `P` is None, it returns `np.quantile(x,quantiles,axis)`.
* `median(P,x,axis)` computes the median `x` using the distribution `P`. If `P` is None, it returns `numpy.std(x,axis)/sqrt(x.shape[axis])`.
* `err(P,x,axis)` computes the standard error of `x` using the distribution `P`. If `P` is None, it returns `numpy.std(x,axis)/sqrt(x.shape[axis])`.
