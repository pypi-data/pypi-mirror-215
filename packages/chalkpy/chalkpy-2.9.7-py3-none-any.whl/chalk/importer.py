...
def import_all_files_once(
    file_allowlist: Optional[List[str]] = None, project_root: Optional[Path] = None
) -> List[FailedImport]:
    global has_imported_all_files
    if has_imported_all_files:
        return []
    failed = import_all_files(file_allowlist=file_allowlist, project_root=project_root)
...
def get_resolver(resolver_fqn_or_name: str, project_root: Optional[Path] = None) -> Resolver:
    """
    ...
    Resolver
    """
    failed_imports = import_all_files_once(project_root=project_root)
    if failed_imports:
