...
def check_dirpath_owner(dirpath:str):
    whatever = Path(dirpath)
    if whatever.exists():
        return {"Owner": whatever.owner(), "Group": whatever.group()}
    else:
        return {}


def check_directory_exist_and_writable(dirpath, callback: callable = None):
    """
    ...
    """
    if os.path.exists(dirpath) and os.path.isdir(dirpath):  # Check if directory exists
        if os.access(dirpath, os.W_OK):  # Check if directory is writable
            callback(f"The directory {dirpath} exists and is writable.")
        else:
            ownership = check_dirpath_owner(dirpath)  # Get directory ownership
            callback(f"The directory {dirpath} exists but is not writable. Ownership: `{ownership}`")
    else:
        callback(f"The directory {dirpath} does not exist.")
...
def create_new_directory(dirpath, callback:callable=None):
    ...
    try:
        if not os.path.exists(dirpath):
            ...
            if callback:
                callback(f"The directory {dirpath} was created.")
        else:
            if callback:
                callback(f"The directory {dirpath} already exists.")
        return dirpath
    except OSError as e:
...
def load_yaml_from_file(file: os.FileIO):
    """
