...
class Foreverbull(threading.Thread):
    ...
    def __init__(
        ...
        self._routes.add_route(self.configure, "configure", models.Execution)
    ...
    def add_route(func, msg_type):
        for key, value in signature(func).parameters.items():
            if value.annotation == Database or value.annotation == models.OHLC:
                continue
            default = None if value.default == value.empty else str(value.default)
            parameter = models.Parameter(key=key, default=default, type=Foreverbull._eval_param(value.annotation))
            Foreverbull._parameters.append(parameter)
        Foreverbull._routes[msg_type] = func
    ...
    def info(self):
        return models.Info(
            type="worker", version="0.0.1", parameters=self._parameters, routes=list(self._routes.keys())
        )
    ...
    def setup(self, **routes):
        ...
        self._worker_pool = WorkerPool(**routes)
        self._worker_pool.setup()

    def configure(self, configuration: models.Execution) -> None:
