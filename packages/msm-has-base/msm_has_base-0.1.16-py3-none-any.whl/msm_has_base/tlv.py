...
def decode(
    ...
    *,
    ...
    convert: _t.Optional[_t.Callable[[
        str, _t.Union[bytes, bytearray]], _t.Any]] = None,
) -> _t.Dict[str, _t.Any]:
    r"""Decode TLV data.
    ...
    if convert is None:
        def convert(t, v): return bytes(v)
    ...
    while ofst < ofst_limit:
        ...
        if constructed:
            if flatten:
                ...
                ofst = _decode(
                    data, ofst, ofst +
                    tag_len, dec[tag], flatten, simple, convert
                )
        # Primitive data type
        else:
...
def _encode(tlv: _t.Mapping[str, _t.Any], simple: bool) -> bytearray:
    ...
    for tag_s, value in tlv.items():
        ...
        try:
            ...
            raise EncodeError(
                "Invalid tag format, expecting hexchar string", tag_s)
