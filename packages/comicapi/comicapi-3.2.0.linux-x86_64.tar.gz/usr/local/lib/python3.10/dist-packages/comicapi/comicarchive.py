...
from comicapi import filenamelexer
from comicapi import filenameparser
from comicapi import utils
from comicapi.archivers import Archiver
from comicapi.archivers import UnknownArchiver
from comicapi.archivers import ZipArchiver
...
exe_paths: dict[str, str] = defaultdict(str)


def load_archive_plugins(force: bool = False, **exes: str) -> None:
    if not archivers or force:
        archivers.clear()
        exe_paths.update(exes)
...
class ComicArchive:
    ...
    def __init__(self, path: pathlib.Path | str, default_image_path: pathlib.Path | None = None) -> None:
        ...
        for archiver in archivers:
            if archiver.is_valid(self.path):
                self.archiver = archiver.open(self.path, exe_paths[archiver.exe])
                break
    ...
    def get_page_name_list(self, sort_list: bool = True) -> list[str]:
        if not self.page_list:
            ...
            for name in files:
                if (
                    os.path.splitext(name)[1].casefold() in [".jpg", ".jpeg", ".png", ".gif", ".webp", ".bmp"]
