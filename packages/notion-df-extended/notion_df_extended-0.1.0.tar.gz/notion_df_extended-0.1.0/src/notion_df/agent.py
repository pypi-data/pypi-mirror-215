...
from notion_df.blocks import BaseNotionBlock, parse_blocks
from notion_df.configs import NON_EDITABLE_TYPES, DatabaseSchema
from notion_df.utils import flatten_dict, is_uuid
...
def download_df_from_database(
    ...
    if nrows is not None and nrows <= NOTION_MAX_PAGE_SIZE:
        page_size = nrows
...
def upload_to_database(df, databse_id, schema, client, errors, children) -> List[Dict]:
    ...
    for idx, (_, row) in enumerate(
        df[::NOT_REVERSE_DATAFRAME].iterrows(),
    ):
        try:
    ...
    *,
    ...
    Args:
        ...
        client (Client, optional):
            ...
            Defaults to None.
    """
    if schema is None and hasattr(df, "schema"):
        schema = df.schema
        ...
        for col in df.columns:
            if schema[col].type == "relation":
                ...
                if relation_df is not None:
                    ...
                    if (
                        create_new_rows_in_relation_target
                        and len(new_object_strings) > 0
                    ):
                    ...
                    df[col] = df[col].apply(
                        lambda row: [
                            obj_string_to_id[ele]
                            for ele in row
                            if ele in obj_string_to_id
                        ]
                    )
