...
import pandas,vcr,click,datetime
from snowflake.snowpark.functions import col,lit
from snowflake.snowpark import Session
from tabulate import tabulate
from slugify import slugify
from omnata_plugin_runtime.omnata_plugin import OutboundSyncRequest,OmnataPlugin,InboundSyncRequest,SyncRequest
from omnata_plugin_runtime.api import OutboundApplyPayload,InboundApplyPayload
from omnata_plugin_runtime.configuration import OutboundSyncConfigurationParameters,StoredConfigurationValue,StoredMappingValue,OutboundSyncStrategy,SyncConfigurationParameters,InboundSyncConfigurationParameters,SyncDirection
...
class DevelopmentSession:
    ...
    def prepare_outbound_scenario(self,
        ...
        self.current_scenario = SyncScenario(plugin_id=self.plugin_id,
    ...
    def prepare_outbound_scenario_from_sync(self,
        ...
        configuration_parameters = OutboundSyncConfigurationParameters(connection_method=outbound_payload.connection_method,
    ...
    def prepare_inbound_scenario_from_sync(self,
            scenario_name:str,
            sync_slug:str,
            current_stream_state:Dict = {},
            snowcli_environment:str='dev') -> Tuple[InboundSyncConfigurationParameters,InboundSyncRequest]:
        """
        Prepares an outbound sync scenario using a Sync which has been fully configured in the Omnata app.
        The user must have the Omnata application role PLUGIN_DEVELOPER, as well as having develop privileges
        on the plugin app.
        ...
        if 'plugin_app_name' not in response_data:
            raise ValueError(f"Invalid response from CREATE_DEVELOPMENT_SESSION_RUN: {response_data}")
        plugin_app_name = response_data['plugin_app_name']
        inbound_payload = InboundApplyPayload.parse_obj(response_data['inbound_apply_payload'])

        # we have everything we need except the secrets, which we fetch from the plugin app
        oauth_secret_param = f"'{inbound_payload.oauth_secret_name}'" if inbound_payload.oauth_secret_name else 'null'
        other_secrets_param = f"'{inbound_payload.other_secrets_name}'" if inbound_payload.other_secrets_name else 'null'
        ...
        #full_source_table_name = f"{source_database}.{source_schema}.{source_table}"
        #self.api_limits.concurrency = 1 # we have to set this because pyvcr isn't thread safe
        configuration_parameters = InboundSyncConfigurationParameters(connection_method=inbound_payload.connection_method,
                                                    connection_parameters=parse_obj_as(Dict[str,StoredConfigurationValue],inbound_payload.connection_parameters),
                                                    connection_secrets=parse_obj_as(Dict[str,StoredConfigurationValue],secrets_data),
                                                    sync_parameters=parse_obj_as(Dict[str,StoredConfigurationValue],inbound_payload.sync_parameters),
                                                    current_form_parameters={})
        
        streams_list = list(inbound_payload.streams_configuration.included_streams.values())
        for stream in streams_list:
            if stream.stream_name in inbound_payload.latest_stream_state:
                stream.latest_state = inbound_payload.latest_stream_state[stream.stream_name]
    ...
    def _start_recording(self):
        ...
        """
        my_vcr = vcr.VCR(
            before_record_request=scrub_secrets(self.current_scenario.configuration_parameters.connection_secrets),
            record_mode='all',
    ...
    def validate_response(self,sync_request:SyncRequest) -> str:
        ...
        if sync_request.__class__.__name__=='OutboundSyncRequest':
            ...
            if outbound_sync_request._apply_results is None or len(outbound_sync_request._apply_results)==0:
            ...
            if 'APP_IDENTIFIER' in column_list:
                if not pandas.api.types.is_string_dtype(results_dataframe['APP_IDENTIFIER']):
                    print('Failed: "APP_IDENTIFIER" column is not a string type. Use ".astype(str)" to enforce string conversion')
                    return
            ...
            raise ValueError(f'Failed: sync request is not an OutboundSyncRequest or InboundSyncRequest, it is a {sync_request.__class__.__name__}')
    ...
    def generate_behave_test(self):
        ...
        if len(self.recorded_scenarios)==0:
            print('Failed: No recorded scenarios found - did you forget to call complete_scenario()?')
            return
        ...
        for recorded_scenario in self.recorded_scenarios:
            ...
            secrets_redacted = {}
            for secret,value in recorded_scenario.configuration_parameters.connection_secrets.items():
            ...
            if isinstance(recorded_scenario.configuration_parameters,OutboundSyncConfigurationParameters):
                outbound_params:OutboundSyncConfigurationParameters = recorded_scenario.configuration_parameters
                properties_df = pandas.DataFrame([
                    ...
                    {"Property":"connection_method","Value": recorded_scenario.configuration_parameters.connection_method},
                ...
                inbound_params:InboundSyncConfigurationParameters = recorded_scenario.configuration_parameters
                inbound_sync_request:InboundSyncRequest = recorded_scenario.sync_request
                properties_df = pandas.DataFrame([
                    {"Property":"connection_method","Value": recorded_scenario.configuration_parameters.connection_method},
                    ...
                    {"Property":"streams","Value": json.dumps(inbound_sync_request.streams,default=pydantic_json_encoder)}
                ])
            ...
            if recorded_scenario.sync_direction==SyncDirection.OUTBOUND:
                ...
                for stream in inbound_sync_request.streams:
                    if stream.stream_name in stream.latest_state:
                        f.write(f"\t\t\t| {stream.stream_name} | {json.dumps(stream.latest_state,separators=(',', ':'))} | \n")
                    else:
            ...
            if recorded_scenario.sync_direction==SyncDirection.OUTBOUND:
                f.write("\t\tWhen we perform an outbound sync with configuration parameters:\n")
            else:
                f.write("\t\tWhen we perform an inbound sync with configuration parameters:\n")
            f.write(f"{table_indented}\n")
            if recorded_scenario.sync_direction==SyncDirection.OUTBOUND:
                ...
                for stream_name in recorded_scenario.results.keys():
                    f.write(f'\t\tAnd the response for the {stream_name} stream will be:\n')
                    results = pandas.concat(recorded_scenario.results[stream_name])
                    results['RECORD_DATA']=results['RECORD_DATA'].apply(json.dumps)
                    results.loc[results["RECORD_DATA"]=='"null"', "RECORD_DATA"] = 'null'
                    results = results[results.columns.intersection(['APP_IDENTIFIER','RECORD_DATA','RETRIEVE_DATE','STREAM_NAME'])]
