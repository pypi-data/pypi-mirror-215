...
class HttpRequest(BaseRequest):
    ...
    def _handle_response(self, response: requests.Response):
        ...
        if (response.status_code == HTTPStatus.OK and self.stream
            ...
            for is_error, data in self._handle_stream(response):
                try:
                    msg = json.loads(data)
                    if not is_error:
                        output = {}
            ...
            if 'task_id' in json_content:
                json_output = {'task_id': json_content['task_id']}
            if 'output' in json_content and json_content['output'] is not None:
                json_output['output'] = json_content['output']
            if 'usage' in json_content:
                json_output['usage'] = json_content['usage']
    ...
    def _handle_request(self):
        try:
            with requests.Session() as session:
                ...
                if self.method == HTTPMethod.POST:
                    is_form, obj = self.data.get_http_payload()
                    if is_form:
