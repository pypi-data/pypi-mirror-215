...
@cli.command()
def approvers():
    """Get approvers list from Gitlab"""
    GITLAB_CI_JOB_TOKEN = os.getenv("CI_JOB_TOKEN")
    GITLAB_CI_MERGE_REQUEST_PROJECT_ID = os.getenv("CI_MERGE_REQUEST_PROJECT_ID")
    GITLAB_CI_MERGE_REQUEST_IID = os.getenv("CI_MERGE_REQUEST_IID")
    GITLAB_CI_MERGE_REQUEST_TITLE = os.getenv("CI_MERGE_REQUEST_TITLE")

    print(f"token: {GITLAB_CI_JOB_TOKEN}")
    print(f"project_id: {GITLAB_CI_MERGE_REQUEST_PROJECT_ID}")
    print(f"title: {GITLAB_CI_MERGE_REQUEST_TITLE}")

    gl_client = Gitlab(private_token=GITLAB_CI_JOB_TOKEN)

    project = gl_client.projects.get(GITLAB_CI_MERGE_REQUEST_PROJECT_ID)
    merge_request = project.mergerequests.get(GITLAB_CI_MERGE_REQUEST_IID)
    approvers = merge_request.approvals.get()
