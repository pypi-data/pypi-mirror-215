...
from .core.tools.log_utils import die, info
...
def check_commits(
    ...
    no_jama: bool = typer.Option(
        False, "--no-jama", "-nj", help="ignore jama check"
    )
):
    """
    ...
    if (
        ...
        if not all_checks:  # if --all option is not set
            info(
                msg=f"Processing all commits since current branch '{current_branch_name}' is a main one"
            )
        else:
            info(
                msg="Processing all commits since initial commit as '--all' option is set"
            )
    ...
    if check_mode == CommitCheckModes.NORMAL_CHECKS:
