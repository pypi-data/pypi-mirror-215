...
from tests.classes_skels import ClassListNested, ClassListMixDictIn, ClassOldStyle
...
def test_type_check():
    ...
    get_field_with_type(test1, "space") == (123, int)
    test1.space = np.array(123.)
    with pytest.raises(TypeError):
        get_field_with_type(test1, "space")
    test1.space = None
    get_field_with_type(test1, "space") == (None, int)
