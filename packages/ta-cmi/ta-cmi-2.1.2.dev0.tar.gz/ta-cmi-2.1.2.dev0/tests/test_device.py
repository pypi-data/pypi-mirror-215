...
async def test_device_with_more_than_seven_parameters():
    ...
    ) as device_data_mock, patch("asyncio.sleep", wraps=sleep_mock) as sleep_m:
        await device.update()
    ...
    sleep_m.assert_called_once_with(61)
...
@pytest.mark.asyncio
@pytest.mark.parametrize("channel_mode", list(ChannelType._member_map_.values()))
async def test_get_channels(channel_mode):
    ...
    channels = device.get_channels(channel_mode)

    for ch in channels.values():
        assert ch.mode == channel_mode
