...
class HelpFormatter(click.HelpFormatter):
    ...
    ACCENT_REGEX = compile(r"(^|)?\*([\w!/.-]+?)\*")
    LITERAL_REGEX = compile(r"'(\n\s*)?'")
    INLINE_CONFIG_VAR_REGEX = compile(r"(?<!<)<(\n\s*)?([\w\s.-]+?)>")
    ENV_VAR_REGEX = compile(r"\{(\n\s*)?([\w.-]+?)}")  # @FIXME join with other wrappers?
    INLINE_ENV_VAR_REGEX = compile(r"@([()\w._-]+?)@")  # @FIXME join with other wrappers?
    ABOMINATION_REGEX = compile(r"::([\w./-]+?:*)::")
    DEPENDENCY_REGEX = compile(r"\+\+([\w./-]+?:*)\+\+")
    COMMENT_REGEX = compile(r"(\s//\s?)(.+)($)")
    COMMENT_BLOCK_REGEX = compile(r"/\*()(.+)()\*/", flags=re.DOTALL)
    ...
    def __init__(
        ...
        self.help_filters: t.List[pt.StringReplacer] = [
            ...
            RegexValRefilter(
                compile(r"&([A-Z](?=[^A-Z])|[a-z](?=[^a-z]))"),
                HelpStyles.TEXT_ABBREV,  # swapcase
            ),
        ]
