...
class TestCommand:
    @pytest.mark.parametrize(
        ...
        [
            ...
            ["exec gradient --example", "solid-cherry"],
            ["exec hilight-num --example", "http://localhost:8087/"],
            ["exec l4 /", re.compile("([r-][w-][x-]){3}")],
            ["exec ls /", re.compile("([r-][w-][x-]){3}")],
            pytest.param("exec notify test", "", marks=[pytest.mark.skip()]),
            ["exec strdbg send /dev/zero", re.compile("Dusk|Now")],
            ...
            ["print env-hosts", "Files found"],
            ["print keys tmux --details", "send-keys"],
            ["print keys x11 --details", "send-keys"],
...
class TestCliCommonOptions:
    ...
    def test_sgrs_in_output(self, io_injector):
        _, stdout, _ = io_injector.run_expect_ok(["exec", "hilight-num", "--example"])
