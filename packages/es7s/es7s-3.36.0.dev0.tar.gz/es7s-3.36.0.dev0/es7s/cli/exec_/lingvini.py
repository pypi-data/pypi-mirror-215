...
class LangBreakdownCommand:
    ...
    def _render_dir_info(self, data: dict, absp: Path, repo: GitRepo, title: str):
                ...
                if str(absp) == os.path.commonpath([absp, Path(stat_file)]):
                    if os.path.isfile(stat_file):
                        try:
                            ...
                            logger.non_fatal_exception(f"Failed to count lines: '{stat_file}': {e}")
        ...
        for lang_data in sorted(data_filtered, key=lambda v: -v.get("size")):
