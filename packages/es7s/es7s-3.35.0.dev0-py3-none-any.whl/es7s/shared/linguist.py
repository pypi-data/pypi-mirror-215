# ------------------------------------------------------------------------------
#  es7s/core
#  (c) 2023 A. Shavykin <0.delameter@gmail.com>
# ------------------------------------------------------------------------------
import os
from pathlib import Path

from es7s.shared import Styles, get_logger, get_stdout, run_subprocess
from es7s.shared.exception import SubprocessExitCodeError
from es7s.shared.path import DOCKER_PATH, GH_LINGUIST_PATH


class Linguist:
    @classmethod
    def get_lang_statistics(cls, path: Path|str, docker: bool, breakdown: bool) -> str|None:
        args = cls._get_linguist_args(path, docker, breakdown)
        return cls._invoke_linguist(args)

    @classmethod
    def _invoke_linguist(cls, args: list[str | Path]) -> str:
        cp = run_subprocess(*args, check=False)
        if cp.returncode != 0:
            if get_logger().verbosity_allows_subprocess_stderr:
                get_stdout().echo_rendered(cp.stderr, Styles.ERROR)
            raise SubprocessExitCodeError(cp.returncode, args)
        return cp.stdout

    @classmethod
    def _get_linguist_args(cls, path: Path, docker: bool, breakdown: bool) -> list:
        opts = []
        if breakdown:
            opts = ['-b']

        if not docker:
            return [GH_LINGUIST_PATH, *opts, path]
        return [
            DOCKER_PATH,
            "run",
            "--rm",
            "-v",
            f"{path}:/data",
            "-w",
            "/data",
            "-u",
            f"{os.getuid()}:{os.getgid()}",
            "-t",
            "github-linguist",
            "github-linguist",
            *opts,
            "/data",
        ]
