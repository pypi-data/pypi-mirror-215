...
from ...shared.demo import DemoGradients
from ...shared.gradient import GimpGradientReader, GradientPoint
...
@cli_option(
    ...
    "the details amount even further ('-dd' or '-ddd').",
)
@cli_option(
    "-X",
    "--example",
...
class DemoGradientCommand:
    ...
    def __init__(
        self, height: int, details: int, example: bool, file: tuple[TextIO, ...] | None, **kwargs
    ):
        if file is None:
    ...
    def _get_details_renderer(self) -> IDetailsRenderer | None:
        if self._details > 0:
            return PrecalcDetailsRenderer(self._details, self._width)
        return None
