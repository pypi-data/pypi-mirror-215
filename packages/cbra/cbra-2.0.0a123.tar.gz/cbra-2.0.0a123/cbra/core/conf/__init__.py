...
_RDBMS_CONFIG_FILE: str = os.getenv('RDBMS_CONNECTIONS') or '/etc/secrets/rdbms.yml'


def parse_sql_connections() -> dict[str, Any]:
    if not os.path.exists(_RDBMS_CONFIG_FILE):
        return {}
    connections: dict[str, Any] = {}
    for params in yaml.safe_load(open(_RDBMS_CONFIG_FILE)):
        connections[params.pop('connection')] = {
            str.upper(k): v for k, v in params.items() if k != 'connection'
        }
    return connections
...
class Settings:
    ...
    __defaults__: dict[str, Any] = {
        ...
        'DATABASES': parse_sql_connections(),
        'DEBUG': False,
