...
class Packages(List):
    ...
    def create(self, filename: Path, **kwargs) -> Package:
        """
...
class Algo(Object):
    ...
    def upload(self) -> Package:
        ...
        with local.Package(ik_algo) as zf:
            return self.packages.create(
                zf, ikomia_min_version=ik_algo.info["ikomia_version"], **local.get_platform_information()
            )
...
class Algos(List):
    ...
    def create(self, icon: Path, **kwargs) -> Algo:
        """
        Create a new algo.
