...
evaluate_component = command(
    ...
    code="./src",
    command="python -m train_computer_vision_model.evaluate_script --model_path ${{inputs.model}}"
    " --test_dataset_dir ${{inputs.test_dataset_dir}} --project ${{inputs.project}}"
    " --mode ${{inputs.mode}} --output_dir ${{outputs.evaluation_outputs}}",
    environment="robotic_manipulation2:18",
    experiment_name=f"{default_experiment_name}",
)
...
def training_pipeline(model_weight: str, data_path: str) -> None:
    train_step = train_component(
        ...
        epochs=Uniform(min_value=1, max_value=3),
...
YOLOmodel = Input(
    path="azureml://subscriptions/0a94de80-6d3b-49f2-b3e9-ec5818862801/resourcegroups/buas-y2"
    "/workspaces/MLOpsRobotics/datastores/workspaceblobstore/paths/data/Yolo Models/yolov8n.pt",
    type="uri_file",
