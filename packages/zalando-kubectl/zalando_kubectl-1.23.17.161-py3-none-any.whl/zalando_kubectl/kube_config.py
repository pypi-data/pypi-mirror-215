...
def get_auth(env: Environment, cluster, stdout=None, stderr=None):
    ...
    return env.kubelogin.run(cmd, stdout=stdout, stderr=stderr)
...
def get_auth_token(env, alias):
    token_bytes = get_auth(env, alias, stdout=PIPE, stderr=PIPE).stdout
...
def is_okta_user():
    ...
    try:
        for c in config["contexts"]:
            return c["context"]["cluster"] == config["current-context"] and "okta" in c["context"]["user"]
    except KeyError:
        return False
