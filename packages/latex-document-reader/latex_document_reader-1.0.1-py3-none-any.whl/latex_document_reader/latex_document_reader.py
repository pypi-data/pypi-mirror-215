...
class LaTeXDocumentReader:
    ...
    def convert_tex_to_pdf(self, tex_file_path, output_file):
        # Convert the tex file to pdf
        try:
            output_file = output_file if output_file.endswith('.pdf') else output_file + '.pdf'
            doc = Document()
            doc.append(Command('input', tex_file_path))
