...
class QUBOSpecBuilder(SpecBuilder):
    ...
    def root_node_only(self):
        ...
        Setting this number to 1 only solves the root node.
        This mimicks the behavior of ordinary (quantum) annealers which "only" return a solution without any quality assessment or proof of global optimality.
        When comparing to other quantum annealers, make sure to limit the number of nodes to 1.If the solution process is stopped after the maximum number of nodes is reached, our solver still returns an optimality gap which tells you how far away the true optimal solution potentially is.
        This is not the case with other ordinary (quantum) annealers.
