...
def test_empty_string_sequence_loader(tmp_datasets_dir):
    ds = deeplake.dataset(tmp_datasets_dir / "empty_string_sequence", overwrite=True)
    with ds:
...
def test_pil_decode_data_loader(tmp_datasets_dir):
    ds = deeplake.dataset(tmp_datasets_dir / "pil_dataset", overwrite=True)
    with ds:
        ds.create_tensor(
            "images",
            dtype=np.uint8,
...
def test_sequence_pil_decode_data_loader(tmp_datasets_dir):
    ds = deeplake.dataset(tmp_datasets_dir / "pil_sequence_dataset", overwrite=True)
    with ds:
        ds.create_tensor(
            "images",
            dtype=np.uint8,
...
def test_json_data_loader(tmp_datasets_dir):
    ds = deeplake.dataset(tmp_datasets_dir / "json_dataset", overwrite=True)
    with ds:
        ds.create_tensor(
            "json",
            htype="json",
...
def test_list_data_loader(tmp_datasets_dir):
    ds = deeplake.dataset(tmp_datasets_dir / "list_dataset", overwrite=True)
    with ds:
        ds.create_tensor(
            "list",
            htype="list",
...
def test_hidden_tensors_data_loader(tmp_datasets_dir):
    ds = deeplake.dataset(tmp_datasets_dir / "hidden_tensors_iteration", overwrite=True)
    with ds:
        ds.create_tensor(
            "images",
            sample_compression=None,
    ...
    cpp_ds = api.dataset(str(tmp_datasets_dir / "hidden_tensors_iteration"))
    dl = iter(Loader(
        cpp_ds,
        batch_size=1,
