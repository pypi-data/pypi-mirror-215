...
class Session(WithState):
    ...
    @property
    def datajoint_path(self) -> pathlib.Path:
        return paths.DATAJOINT_ROOT / 'ks_paramset_idx_1' / str(self)
        
    @cached_property
    def metrics_csv(self) -> tuple[pathlib.Path, ...]:
        for path in (self.npexp_path, self.datajoint_path):
            csvs = tuple(self.npexp_path.rglob('metrics.csv'))
            if csvs:
                return csvs
        return ()
