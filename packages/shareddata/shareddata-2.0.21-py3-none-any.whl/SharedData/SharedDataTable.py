...
class SharedDataTable:
    ...
    def __init__(self, sharedDataFeeder, dataset, records=None,\
        ...
        self.keys = SharedDataTableKeys(self)
    ...
    def free(self):
        if os.name=='posix':
            path, shm_name = self.get_path(iswrite=True)            
            self.free_shm(shm_name)
            self.free_shm(shm_name+'#pkey')
            self.free_shm(shm_name+'#dateiniidx')
            self.free_shm(shm_name+'#dateendidx')            
            self.free_shm(shm_name+'#portiniidx')
            self.free_shm(shm_name+'#portendidx')
            self.free_shm(shm_name+'#portlist')            
            self.free_shm(shm_name+'#portlistcount')

    def free_shm(self,shm_name):
        try:
    ...
    def register_access(self,shm_name):
        fpath = Path(os.environ['DATABASE_FOLDER'])/('shm/'+shm_name.replace('\\','/')+'.csv')
