...
class SharedNumpy(np.ndarray):
    ...
    def upsert(self, new_records):
        if new_records.size>0:
            ...
            try:
                ...
                minchgid = self.count
                arr = super().__getitem__(slice(0, self.size))
                if (self.table.sharedData.database=='Risk')\
                    | (self.table.sharedData.database=='Positions'):
            ...
            if len(shift_idx) > 0:
                ...
                if np.where(idx_diff != 1)[0].any():
                    ...
                    if not self.keys.initialized:
                        ...
                        self.keys.update_pkey(_minchgid)
        except Exception as e:
    ...
    def get_date_portfolio_loc(self,keys):    
        if not self.keys.initialized:
            self.keys.initialize()    
        return self.keys.get_date_portfolio_loc_func(self[:], keys, self.pkey, self.keys.portiniidx,self.keys.portlist)
