...
def allTaskState(lock):
    ...
    for it in datas:
        ...
        if it["state"] == C_STATE.DOWNLOADING or it["state"] == C_STATE.WAIT_DOWNLOAD:
            state_list["downloading"] = state_list["downloading"] + " " + str(it["curGroupId"])
        if it["state"] == C_STATE.TEMPLATEING or it["state"] == C_STATE.WAIT_TEMPLATE:
            state_list["processing"] = state_list["processing"] + " " + str(it["curGroupId"])
        if it["state"] == C_STATE.UPLOADING or it["state"] == C_STATE.WAIT_UPLOAD:
