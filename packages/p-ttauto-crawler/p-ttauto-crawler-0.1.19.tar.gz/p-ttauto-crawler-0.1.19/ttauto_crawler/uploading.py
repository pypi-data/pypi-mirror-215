...
def notifyMessage(curGroupId, ossurl, count):
    try:
        ...
        if res.status_code == 200:
            utils.logInfo(f"notifyMessage success", False)
        else:
            resContext = res.content.decode(encoding="utf8", errors="ignore")
            utils.logInfo(f"notifyMessage fail! code={res.status_code}, context={resContext}", False)
            utils.logInfo(f"report error!, postdata = {json.dumps(param)}", False)
        s.close()
    except Exception as e:
        utils.logInfo(f"notifyMessage exception :{e}", False)
...
def upload(processed_dir, curGroupId):
    ...
    for rt,dirs,files in os.walk(processed_dir):
        for file in files:
            ...
            if splitCount >= splitZipCount and (len(files) - successCount > (splitZipCount / 2)):
                ...
                utils.logInfo(f"=== sending {packageIndex} package ", False)
    ...
    if splitCount > 0:
        ...
        utils.logInfo(f"=== sending last package ", False)
