...
def allTaskState(lock):
    ...
    for it in datas:
        ...
        if it["state"] == C_STATE.DOWNLOADING.value or it["state"] == C_STATE.WAIT_DOWNLOAD.value:
            state_list["downloading"] = state_list["downloading"] + " " + str(it["curGroupId"])
        if it["state"] == C_STATE.TEMPLATEING.value or it["state"] == C_STATE.WAIT_TEMPLATE.value:
            state_list["processing"] = state_list["processing"] + " " + str(it["curGroupId"])
        if it["state"] == C_STATE.UPLOADING.value or it["state"] == C_STATE.WAIT_UPLOAD.value:
