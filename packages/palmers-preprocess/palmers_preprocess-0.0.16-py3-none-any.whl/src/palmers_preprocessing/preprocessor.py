    ...
    def preprocess(cls, stores_list: list[int] = global_config.OUTLETS_SDATTA, marketing_plan: pd.DataFrame = None,
        ...
        for store in stores_list:
            ...
            for item in store_data_df[global_config.ITEM_COLUMN_NAME].unique():
                ...
                temp["roll_3"] = temp[[global_config.DATE_COLUMN_NAME, global_config.ITEM_STORE_COLUMN_NAME,
                                    global_config.SALES_COLUMN_NAME]].groupby(
                    [global_config.ITEM_STORE_COLUMN_NAME])[
                    global_config.SALES_COLUMN_NAME].rolling(2).mean().reset_index(0, drop=True)

                temp[global_config.SALES_COLUMN_NAME] = temp[global_config.SALES_COLUMN_NAME] * 6
