...
class SQLiteDataStorage(AbstractDataStorage):
    ...
    def __init__(self, db_file: Optional[str] = None, uri: str = ""):
        ...
        try:
            ...
            self._reflect_tables(
                prefixes=[self.LISTING_TABLE_NAME_PREFIX, self.DATASET_TABLE_PREFIX]
            )
        except RuntimeError:
    ...
    def _reflect_tables(self, prefixes: Optional[List[str]] = None):
        """
        ...
        If prefixes is defined then only tables which names start with one of the
        prefixes is reflected, and if it's not defined then all tables are reflected
        """

        def check_table(table_name, _):
            return not prefixes or any(
                [table_name.startswith(prefix) for prefix in prefixes]
            )
    ...
    def add_bucket_signal_column(self, uri: str, col_name: str, col_type: "TypeEngine"):
        ...
        self._reflect_tables(prefixes=[nodes.name])
    ...
    def get_table(self, name: str) -> sqlalchemy.Table:
        ...
        self._reflect_tables(prefixes=[name])
