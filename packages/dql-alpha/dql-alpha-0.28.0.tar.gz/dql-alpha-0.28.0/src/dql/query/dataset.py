...
class IndexingStep(StartingStep):
    ...
    def apply(self):
        ...
        node_custom_cols = self.catalog.data_storage.nodes_table(uri).custom_columns
        dataset_column_names = list(DATASET_CORE_COLUMN_NAMES) + [
            c.name for c in node_custom_cols
        ]
        dataset_columns = [sqlalchemy.column(c_name) for c_name in dataset_column_names]
