...
class SensorParser:
    ...
    def _data_len_from_frc_command(frc_command: int):
        if frc_command == SensorFrcCommands.FRC_2BITS or frc_command == SensorFrcCommands.FRC_1BYTE:
            return 1
    ...
    def frc_dpa(cls, sensor_type: Union[SensorTypes, int], sensor_index: int, frc_command: int, data: List[int],
                extra_result: Union[List[int], None] = None):
        dpa = data[2:]
        ...
        for index in range(0, len(dpa), data_len):
            match data_len:
                case 1:
                    frc_value = dpa[index]
                case 2:
                    frc_value = dpa[index] << 8 + dpa[index + 1]
                case _:
                    frc_value = (dpa[index] << 24) + (dpa[index + 1] << 16) + (dpa[index + 2] << 8) + dpa[index + 3]
            sensor_data.append(
                SensorData(
                    sensor_type=sensor_type,
                    index=sensor_index,
