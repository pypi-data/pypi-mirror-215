...
class ResourceObject(AbstractJsonObject):
    ...
    @property
    def url(self) -> str:
        # keep only self link URL path and remove query
        url = urljoin(str(self.links.self), urlparse(str(self.links.self)).path)
        if self.type is not None:
            type_path:str = self.type.replace('--', '/')
        else:
            type_path:str = self.type
        return url or self.id and f'{self.session.url_prefix}/{type_path}/{self.id}'
    ...
    def post_url(self) -> str:
        if self.type is not None:
            ...
            type_path:str = self.type
        return f'{self.session.url_prefix}/{type_path}'
    ...
    def _commit_file_sync(
        ...
        url = self._pre_commit(url)
        status, result, location = self.session.http_request(
            self._http_method,
            url,
            send_data = send_data,
            expected_statuses = expected_statuses,
    ...
    def commit_file(self, custom_url: str = '', meta: dict = None,
                    send_data: IO[bytes] = None, filename: str = None) \
            -> 'Union[None, ResourceObject, Awaitable[Optional[ResourceObject]]':
        """
        Commit (POST) this resource to server.

        :param custom_url: Use this url as it is not possible automatically determined one.
        :param send_data: Bytes data that is passed to server in POST request.
        :param filename: Use this filename instead of automatically determined one.
        :param meta: Optional metadata that is passed to server in POST/PATCH request.

        If in async mode, this needs to be awaited.
        """
        filename = filename or self.fields['filename']
        if not filename:
            raise ValidationError('The filename for uploading file is required.')
        headers = {
            'Content-Type':'application/octet-stream',
            'Content-Disposition': f'file; filename="{filename}"',
        }
        expected_statuses = (HttpStatus.CREATED_201,)
        if not custom_url:
            raise ValidationError('The custom_url link for uploading file is missing.')
        if not custom_url:
            raise ValidationError('The send_data for uploading file is missing.')
        if self.session.enable_async:
            return self._commit_file_async(
                custom_url,
                meta,
                send_data = send_data,
                headers = headers,
