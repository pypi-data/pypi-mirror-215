...
{% elif data.configure_spoke_advertisement %}
  ...
  included_advertised_spoke_routes  = var.switch_traffic ? (var.spoke_adv != "" ? var.spoke_adv : join(",", var.vpc_cidr)): join(",", var.avtx_cidrs)
  {% else %}
  included_advertised_spoke_routes  = var.switch_traffic ? (var.spoke_adv != "" ? var.spoke_adv : join(",", var.vpc_cidr)): var.avtx_cidr
  {% endif %}
{% else %}
  {% if data.configure_spoke_gw_hs %}
  included_advertised_spoke_routes  = var.switch_traffic ? join(",", var.vpc_cidr) : join(",", var.avtx_cidrs)
  {% else %}
...
{% if data.configure_spoke_gw_hs %}
resource "aviatrix_spoke_ha_gateway" "ha" {
  count           = local.subnet_count - 1
  primary_gw_name = aviatrix_spoke_gateway.gw.id
  gw_name         = "${var.spoke_gw_name}-ha-${count.index + 1}"
