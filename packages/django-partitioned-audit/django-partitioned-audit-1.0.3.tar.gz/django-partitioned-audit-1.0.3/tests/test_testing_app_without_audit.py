...
def test_create_customer_works_with_audit_disabled(live_server, disable_triggers):
    name = str(uuid.uuid4())
...
@pytest.mark.django_db(transaction=True)
def test_update_customer_works_with_audit_disabled(live_server, disable_triggers):
...
@pytest.mark.django_db(transaction=True)
def test_create_invoice_works_with_audit_disabled(live_server, disable_triggers):
    name = str(uuid.uuid4())
...
@pytest.mark.django_db(transaction=True)
def test_create_product_works_with_audit_disabled(live_server, disable_triggers):
    name = str(uuid.uuid4())
