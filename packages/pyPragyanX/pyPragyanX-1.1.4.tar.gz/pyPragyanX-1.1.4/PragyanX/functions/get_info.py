...
"""
   ...
   from PragyanX.functions import get_info
   await get_info(bot, message)
"""

from PragyanX import *

async def get_info(Pragyan, message):
   try:
...
"""
   ...
   from Pragyan.functions import get_id
   await get_id(bot, message)
"""

async def get_id(Pragyan, message):
    chat = message.chat
    ...
    if len(message.command) == 2:
        try:
            ...
            user = await Pragyan.get_users(split)
    ...
    if not getattr(reply, "empty", True) and not message.forward_from_chat and not reply.sender_chat:
      try:
        user = await Pragyan.get_users(reply.from_user.id)
