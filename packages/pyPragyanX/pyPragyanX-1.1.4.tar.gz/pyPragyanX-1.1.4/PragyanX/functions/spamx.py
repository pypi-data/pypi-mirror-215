"""
   © PragyanX - SpamX
   functions of PragyanX/SpamX
"""

from PragyanX.developers import *
from PragyanX.data import *
...
async def user_reason(RiZoeL, message, Owner, Sudos):
    ...
    if int(user.id) in Devs:
        await message.reply_text(f"{user.mention} is Owner/Dev of @PragyanX")
        return
...
async def start_raid(RiZoeL, message, counts, user):
   ...
   if chat.id in res_grps:
      ...
      return
   if int(user.id) in Devs:
      await message.reply_text("I can't raid on @PragyanX's Owner")
      return
...
async def start_banall(RiZoeL, message):
   ...
   async for u in RiZoeL.get_chat_members(chat.id):
      ...
      try:
         ...
         print(f"pyPragyanX - [INFO]: {str(err)}")
         failed += 1
   await x.delete()
...
def start_spamX(PragyanX, type):
    if type == "token":
      PragyanX.start()
      try:
         x = PragyanX.get_me()
         print(f"pyPragyanX - [INFO]: @{x.username} started ✓")
      except:
         print(f"pyPragyanX - [INFO]: Bot started ✓")
    else:
      PragyanX.start()
      try:
         PragyanX.join_chat("PragyanX")
         PragyanX.join_chat("DNHxHELL")
      except:
         pass
      try:
         x = PragyanX.get_me()
         print(f"pyPragyanX - [INFO]: @{x.first_name} started ✓")
      except:
         print(f"pyPragyanX - [INFO]: Client started ✓")
