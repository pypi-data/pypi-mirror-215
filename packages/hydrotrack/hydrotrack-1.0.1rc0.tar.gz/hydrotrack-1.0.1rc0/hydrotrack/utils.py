...
def get_filestamp(name_list, path_file):
    ...
    regex_stamp = r"(\%YYYY|\%YY)|(\%mm)|(\%dd)|(\%HH)|(\%MM)|(\%SS)"
    # Get the position of the timestamp
    for ptrn in name_list['timestamp_pattern']:
        matches = re.finditer(regex_stamp, ptrn)
        if matches:
            break
    # % counter
    perc_cnt = 0
    ...
    for match in matches:
        if match:
            start_pos = match.start() - perc_cnt
            end_pos = match.end() - perc_cnt - 1
            datestamp = datestamp + (file_string[start_pos:end_pos])
            pstamp = pstamp +  match.group()[:2]
            perc_cnt += 1
    timestamp = datetime.strptime(datestamp, pstamp)
    return timestamp
...
def get_columns():
    ...
    main_columns['features'] = ['timestamp','cluster_id', 'threshold', 'threshold_level', 
                                ...
                                'file', 'geometry']
    main_columns['spatial'] = ['status','prev_idx','merge_idx','split_idx','inside_idx','inside_clusters','dis_', 'dir_','trajectory']
...
def read_geodataframe(path_file, error_columns, attempt=2):
    ...
    while attempt > 0:
        try:
            geoframe = gpd.read_feather(path_file)
            attempt = -1
        except:
...
def get_edge_limit(name_list, files_list, read_function):
    ...
    if name_list['edge_limit'] == 'right' or name_list['edge_limit'] == 'left' or name_list['edge_limit'] == 'top' or name_list['edge_limit'] == 'bottom':
        for files in range(len(files_list)):
            file_path = gpd.read_feather(files_list[files]) # Read the file
            if len(file_path) > 0:
        ...
        if name_list['edge_limit'] == 'right':
            board_1 = gpd.GeoDataFrame({'geometry': [LineString([(data_shape[1], 0),(data_shape[0], data_shape[0])])]})
            ...
            board_2 = gpd.GeoDataFrame({'geometry': [LineString([(data_shape[1], 0),(data_shape[0], data_shape[0])])]})
