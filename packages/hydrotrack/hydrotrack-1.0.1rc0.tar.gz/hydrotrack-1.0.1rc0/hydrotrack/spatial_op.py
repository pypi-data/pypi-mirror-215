...
from .utils import get_columns, read_geodataframe, get_edge_limit, calculate_direction, loading_bar, save_feather, find_previous_frame
import warnings
warnings.simplefilter(action='ignore', category=FutureWarning)

def spatial_operations(name_list, read_function):
    print('Processing the spatial operations...')
    ...
    # Call the function to process the time stamps in parallel    
    Parallel(n_jobs=name_list['n_jobs'])(delayed(process_time)(ctime, tstamp_list, files_list, name_list, board_1, board_2, data_shape) for ctime in range(len(tstamp_list)))
    loading_bar(len(tstamp_list), len(tstamp_list))
    print('\tDone!')

def process_time(current_time, timestamp_list, files_list, name_list, board_1, board_2, data_shape):
    ...
    current_file = features_path + current_timestamp.strftime('%Y%m%d_%H%M%S') + '.feather' # Get the current file
    ...
    if current_frame.empty:
        save_feather(current_frame, current_file)
        return
    ...
    # Save the result
    save_feather(result, current_file)
    loading_bar(current_time, len(timestamp_list))
...
def edge_limit(main_features, board_1, board_2, name_list, shape):
    ...
    geo_df["geometry"] = geo_df["geometry"].buffer(0.5)
    ...
    if len(joined_1) > 0:
        ...
        for idx, idx2, geom in zip(index_1, index_2, unary_geoms):
            ...
            new_trajectory = [loads(main_features.loc[idx, 'trajectory']), loads(contained_1.loc[idx, 'trajectory']), contained_2.loc[idx2, 'trajectory']]
            new_trajectory = [trajectory for trajectory in new_trajectory if not trajectory.is_empty]
            if len(new_trajectory) > 1:
                new_trajectory = str(linemerge(new_trajectory))
            else:
                new_trajectory = str(main_features.loc[idx, 'trajectory'])
            main_features.loc[idx, 'trajectory'] = new_trajectory
        # Create touch_idx column in main_features
        main_features.loc[index_2, 'touch_idx'] = index_1
        # Clean index_2 in main_features
        main_features.loc[main_features.index.isin(index_2), 'prev_idx'] = -1
        main_features.loc[main_features.index.isin(index_2), 'inside_idx'] = None
        main_features.loc[main_features.index.isin(index_2), 'inside_clusters'] = None
        main_features.loc[main_features.index.isin(index_2), 'status'] = None
        main_features.loc[main_features.index.isin(index_2), 'merge_idx'] = None
        main_features.loc[main_features.index.isin(index_2), 'split_idx'] = None
        main_features.loc[main_features.index.isin(index_2), 'dis_'] = None
        main_features.loc[main_features.index.isin(index_2), 'dir_'] = None
        main_features.loc[main_features.index.isin(index_2), 'size'] = None
        main_features.loc[main_features.index.isin(index_2), 'min'] = None
        main_features.loc[main_features.index.isin(index_2), 'max'] = None
        main_features.loc[main_features.index.isin(index_2), 'std'] = None
        main_features.loc[main_features.index.isin(index_2), 'mean'] = None
        main_features.loc[main_features.index.isin(index_2), 'Q1'] = None
        main_features.loc[main_features.index.isin(index_2), 'Q2'] = None
        main_features.loc[main_features.index.isin(index_2), 'Q3'] = None
        main_features.loc[main_features.index.isin(index_2), 'status'] = 'BORDER'
        main_features.loc[main_features.index.isin(index_2), 'geometry'] = loads('POLYGON EMPTY')
        main_features.loc[main_features.index.isin(index_2), 'trajectory'] = 'LINESTRING EMPTY'
