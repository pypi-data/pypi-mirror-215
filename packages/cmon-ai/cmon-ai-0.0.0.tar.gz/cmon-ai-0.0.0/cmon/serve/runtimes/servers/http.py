...
class FastAPIBaseServer(BaseServer):
    ...
    async def setup_server(self):
        ...
        if 'CICD_CMON_DISABLE_HEALTHCHECK_LOGS' in os.environ:
        ...
        if docarray_v2:
            from cmon.serve.runtimes.gateway.request_handling import (
        ...
        self.server = UviServer(
            config=Config(
                ...
                host=self.host,
                port=self.port,
                log_level=os.getenv('CMON_LOG_LEVEL', 'error').lower(),
                **self.uvicorn_kwargs,
            )
        )
...
def _install_health_check(app: 'FastAPI', logger):
    ...
    if not health_check_exists:
        ...
        @app.get(
            path='/',
            summary='Get the health of Cmon Gateway service',
