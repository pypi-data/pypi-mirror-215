...
from cmon.proto.cmon_pb2 import DataRequestProto, CmonInfoProto, RouteProto, StatusProto
...
def protobuf_to_pydantic_model(
    ...
    :param protobuf_model: message from cmon.proto file
    ...
    for f in protobuf_fields:
        ...
        if f.message_type:
            if f.message_type.name == 'Struct':
                ...
                # Proto field type: Proto message defined in cmon.proto
...
if not docarray_v2:
    for proto in (RouteProto, StatusProto, DataRequestProto, CmonInfoProto):
        protobuf_to_pydantic_model(proto)
else:
    for proto in (RouteProto, StatusProto, CmonInfoProto):
        protobuf_to_pydantic_model(proto)
...
if not docarray_v2:
    ...
    class CmonRequestModel(BaseModel):
        """
        Cmon HTTP request model.
        """
    ...
    class CmonResponseModel(BaseModel):
        """
        Cmon HTTP Response model. Only `request_id` and `data` are preserved.
        """
    ...
    class CmonEndpointRequestModel(CmonRequestModel):
        """
        Cmon HTTP request model that allows customized endpoint.
        """
