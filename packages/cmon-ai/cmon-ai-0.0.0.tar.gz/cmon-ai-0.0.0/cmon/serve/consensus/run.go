...
import (
    ...
    cmonraft "jraft/cmon_raft"
    pb "jraft/cmon-go-proto"
...
func Run(myAddr string,
    ...
    executorFSM := cmonraft.NewExecutorFSM(executorTarget, LogLevel, raftId)
    ...
    pb.RegisterCmonSingleDataRequestRPCServer(grpcServer, &cmonraft.RpcInterface{
        Executor: executorFSM,
        Raft:     r,
        Logger:   rpc_logger,
    })
    pb.RegisterCmonDiscoverEndpointsRPCServer(grpcServer, &cmonraft.RpcInterface{
        Executor: executorFSM,
        Raft:     r,
        Logger:   rpc_logger,
    })
    pb.RegisterCmonInfoRPCServer(grpcServer, &cmonraft.RpcInterface{
        Executor: executorFSM,
        Raft:     r,
        Logger:   rpc_logger,
    })
    pb.RegisterCmonRPCServer(grpcServer, &cmonraft.RpcInterface{
                                    Executor: executorFSM,
                                    Raft:     r,
                                    Logger:   rpc_logger,
                              })
    ...
    healthpb.RegisterHealthServer(grpcServer, &cmonraft.RpcInterface{
        Executor: executorFSM,
        Raft:     r,
        Logger:   rpc_logger,
    })
...
func add_voter(self *C.PyObject, args *C.PyObject) *C.PyObject {
    logLevel := os.Getenv("CMON_LOG_LEVEL")
    if logLevel == "" {
        logLevel = "INFO"
    }
...
func get_configuration(self *C.PyObject, args *C.PyObject) *C.PyObject {
    ...
    if C.PyArg_ParseTuple_get_configuration(args, &raftId, &raftDir) != 0 {
        ...
        executorFSM := cmonraft.DummyExecutorFSM()

        conf, err := cmonraft.CmonGetConfiguration(config, executorFSM, logs_db, stable_db, file_snapshot, tm.Transport())
        if err != nil {
            C.Py_IncRef(C.Py_None);
            return C.Py_None;
        } else {
