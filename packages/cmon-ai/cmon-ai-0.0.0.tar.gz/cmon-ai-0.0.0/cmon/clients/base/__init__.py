"""Module containing the Base Client for Cmon."""
import abc
import argparse
import inspect
import os
...
from cmon.excepts import BadClientInput
from cmon.helper import T, parse_client, send_telemetry_event, typename
from cmon.logging.logger import CmonLogger
from cmon.logging.predefined import default_logger

if TYPE_CHECKING:  # pragma: no cover
    from cmon.clients.request import GeneratorSourceType
    from cmon.types.request import Request, Response

    InputType = Union[GeneratorSourceType, Callable[..., GeneratorSourceType]]
    CallbackFnType = Optional[Callable[[Response], None]]
from cmon.serve.instrumentation import InstrumentationMixin
...
class BaseClient(InstrumentationMixin, ABC):
    ...
    def _get_requests(
        ...
        if inspect.isasyncgen(self.inputs):
            from cmon.clients.request.asyncio import request_generator
