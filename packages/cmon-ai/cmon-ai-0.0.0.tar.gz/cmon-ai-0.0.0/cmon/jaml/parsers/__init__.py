...
from cmon.excepts import BadYAMLVersion
from cmon.jaml import JAMLCompatible
from cmon.jaml.parsers.base import VersionedYAMLParser
...
def _get_flow_parser():
    from cmon.jaml.parsers.flow.v1 import V1Parser

    return [V1Parser], V1Parser


def _get_exec_parser():
    from cmon.jaml.parsers.executor.legacy import ExecutorLegacyParser

    return [ExecutorLegacyParser], ExecutorLegacyParser


def _get_deployment_parser():
    from cmon.jaml.parsers.deployment.legacy import DeploymentLegacyParser

    return [DeploymentLegacyParser], DeploymentLegacyParser


def _get_gateway_parser():
    from cmon.jaml.parsers.gateway.legacy import GatewayLegacyParser
