...
def _build_lookup_table():
    ...
    def build_invert_index(d, usage='cmon'):
        for k in d['methods']:
            usg = f'{usage} {k["name"]}'
            if 'methods' in k:
...
def _prettyprint_help(d, also_in=None):
    ...
    if d['type'] == 'command':
        print(
            f'''
    {colored(d['name'], attrs='bold')} is a CLI command of Cmon.
    ...
    More info: {d['usage']} --help
        ...
        print(
            f'''
    {colored(d['name'], attrs='bold')} is {colored('an internal CLI of Cmon, should not be used directly', color='yellow') if d['help'] == argparse.SUPPRESS else 'a CLI argument of Cmon'}. 
...
def lookup_and_print(query: str):
    """Lookup argument name in Cmon API and prettyprint the result.
    ...
    if query not in nkw2kw:
        ...
        print(
            ...
            f'maybe it\'s a misspelling or Cmon does not have this argument.'
        )
    else:
        ...
        if len(helps) == 1:
            ...
            print(
                colored(f'Found {len(help_group)} mentions in Cmon API.', attrs='dark')
            )
