...
class DataRequestProto:
    ...
    def SerializeToString(x: 'DataRequest'):
        ...
        os.environ['CMON_GRPC_SEND_BYTES'] = str(
            len(r) + int(os.environ.get('CMON_GRPC_SEND_BYTES', 0))
        )
        return r
    ...
    def FromString(x: bytes):
        ...
        # noqa: DAR201
        """
        os.environ['CMON_GRPC_RECV_BYTES'] = str(
            len(x) + int(os.environ.get('CMON_GRPC_RECV_BYTES', 0))
        )
...
class DataRequestListProto:
    ...
    def SerializeToString(x: 'Union[List[DataRequest], DataRequest]'):
        ...
        return cmon_pb2.DataRequestListProto(requests=protos).SerializeToString()
    ...
    def FromString(x: bytes):
        ...
        # noqa: DAR201
        """
        rlp = cmon_pb2.DataRequestListProto()
        rlp.ParseFromString(x)
        return [DataRequest.from_proto(request) for request in rlp.requests]


class EndpointsProto:
    """Since the serializer is replacing the `cmon_pb2 to know how to exactly serialize messages, this is just a placeholder that
...
class SnapshotStatusProto:
    ...
    def FromString(x: bytes):
        ...
        # noqa: DAR201
        """
        sp = cmon_pb2.SnapshotStatusProto()
        sp.ParseFromString(x)
...
class RestoreSnapshotCommand:
    ...
    def FromString(x: bytes):
        ...
        # noqa: DAR201
        """
        rpcommand = cmon_pb2.RestoreSnapshotCommand()
...
class RestoreSnapshotStatusProto:
    ...
    def FromString(x: bytes):
        ...
        # noqa: DAR201
        """
        rsp = cmon_pb2.RestoreSnapshotStatusProto()
