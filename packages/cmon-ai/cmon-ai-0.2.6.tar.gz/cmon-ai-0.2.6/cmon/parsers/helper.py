...
_SHOW_ALL_ARGS = 'CMON_FULL_CLI' in os.environ
...
def _get_gateway_class(protocol, works_as_load_balancer = False):
    ...
    if protocol == ProtocolType.HTTP and works_as_load_balancer:
        from cmon.serve.runtimes.gateway.load_balancer import LoadBalancerGateway
    ...
    if not args.uses:
        if len(args.protocol) == 1 and len(args.port) == 1:
            ...
            if len(args.port) == 1:
                ...
                raise ValueError(
                    'You need to specify as much protocols as ports if you want to use a cmon built-in gateway'
                )
