...
from cmon.enums import PodRoleType
from cmon.excepts import NoContainerizedError
from cmon.orchestrate.deployments import Deployment
from cmon.orchestrate.deployments.config.helper import (
...
from cmon.orchestrate.helper import generate_default_volume_and_workspace
...
class DockerComposeConfig:
    ...
    class _DockerComposeService:
        def __init__(
            ...
            self.cmon_deployment_name = cmon_deployment_name
        ...
        def get_gateway_config(
            ...
            envs = [f'CMON_LOG_LEVEL={os.getenv("CMON_LOG_LEVEL", "INFO")}']
            if cargs.env:
            ...
            return {
                ...
                'healthcheck': {
                    'test': f'cmon ping gateway {protocol}://127.0.0.1:{cargs.port[0]}',
                    'interval': '2s',
                },
                'environment': envs,
            }
        ...
        def _get_image_name(self, uses: Optional[str]):
            ...
            image_name = os.getenv(
                'CMON_GATEWAY_IMAGE', f'cmonai/cmon:{self.version}-py38-standard'
            )
        ...
        def get_runtime_config(self) -> List[Dict]:
            ...
            for i_rep in range(self.service_args.replicas):
                ...
                config = {
                    ...
                    'command': container_args,
                    'healthcheck': {
                        'test': f'cmon ping executor 127.0.0.1:{cargs.port}',
                        'interval': '2s',
                    },
                    'environment': [
                        f'CMON_LOG_LEVEL={os.getenv("CMON_LOG_LEVEL", "INFO")}'
                    ],
                }
    ...
    def __init__(
        ...
        if not validate_uses(args.uses):
            raise NoContainerizedError(
                ...
                'You need to use a containerized Executor. You may check `cmon hub --help` to see how Cmon Hub can help you building containerized Executors.'
            )
        self.deployments_addresses = deployments_addresses
        ...
        if self.services_args['head_service'] is not None:
            self.head_service = self._DockerComposeService(
                ...
                shard_id=None,
                cmon_deployment_name=self.name,
