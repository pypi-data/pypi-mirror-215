"""Helper functions for clients in Cmon."""

from functools import wraps
from typing import Callable

from cmon.excepts import BadClientCallback, BadServer
from cmon.helper import get_rich_console
...
def pprint_routes(resp: 'Response', stack_limit: int = 3):
    ...
    for route in routes:
        status_icon = '🟢'
        if route.status.code == cmon_pb2.StatusProto.ERROR:
            status_icon = '🔴'
...
def callback_exec(
    ...
    """
    if response.header.status.code >= cmon_pb2.StatusProto.ERROR:
        if on_error:
    ...
    elif on_done and response.header.status.code == cmon_pb2.StatusProto.SUCCESS:
