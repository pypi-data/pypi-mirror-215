...
from cmon.constants import __windows__
...
def deprecated_alias(**aliases):
    ...
    :return: wrapper
    """
    from cmon.excepts import NotSupportedError
...
def reset_ports():
    def _get_unassigned_ports():
        ...
        if MAX_PORT - DEFAULT_MIN_PORT - len(assigned_ports) < 100:
            min_port = int(os.environ.get('CMON_RANDOM_PORT_MIN', '16384'))
        else:
            min_port = int(
                os.environ.get('CMON_RANDOM_PORT_MIN', str(DEFAULT_MIN_PORT))
            )
        max_port = int(os.environ.get('CMON_RANDOM_PORT_MAX', str(MAX_PORT)))
...
def colored(
    ...
    :return: Colored text.
    """
    if 'CMON_LOG_NO_COLOR' not in os.environ:
        fmt_str = '\033[%dm%s'
        if color:
...
def warn_unknown_args(unknown_args: List[str]):
    ...
    :param unknown_args: arguments that are possibly unknown to Cmon
    """

    from cmon_cli.lookup import _build_lookup_table
    ...
    for arg in unknown_args:
        if arg.replace('--', '') not in all_args:
            from cmon.parsers.deprecated import get_deprecated_replacement
...
def is_valid_local_config_source(path: str) -> bool:
    ...
    """
    try:
        from cmon.jaml import parse_config_source
...
def get_full_version() -> Optional[Tuple[Dict, Dict]]:
    """
    Get the version of libraries used in Cmon and environment variables.
    ...
    from cmon import __docarray_version__, __proto_version__, __version__
    from cmon.constants import __cmon_env__, __unset_msg__, __uptime__
    from cmon.logging.predefined import default_logger
    ...
    try:
        ...
        info = {
            ...
            'cmon-hubble-sdk': __hubble_version__,
            'cmon-proto': __proto_version__,
...
def format_full_version_info(info: Dict, env_info: Dict) -> str:
    ...
    :param info: Version information of Cmon libraries.
    :param env_info: The Cmon environment variables.
...
def _update_policy():
    ...
    elif 'CMON_DISABLE_UVLOOP' in os.environ:
        return
    else:
        try:
            import uvloop
            ...
            warnings.warn(
                'Install `uvloop` via `pip install "cmon[uvloop]"` for better performance.'
            )
...
def run_async(func, *args, **kwargs):
    ...
    if loop and loop.is_running():
        ...
        try:
            ...
            from cmon.excepts import BadClient
...
def extend_rest_interface(app: 'FastAPI') -> 'FastAPI':
    """Extend Cmon built-in FastAPI instance with customized APIs, routing, etc.

    :param app: the built-in FastAPI instance given by Cmon
...
def get_request_header() -> Dict:
    ...
    header = {
        **{f'cmonmeta-{k}': str(v) for k, v in metas.items()},
        **envs,
    }
    return header
...
def get_rich_console():
    """
    Function to get cmon rich default console.
    :return: rich console
    """
    return Console(
        force_terminal=True if 'PYCHARM_HOSTED' in os.environ else None,
        color_system=None if 'CMON_LOG_NO_COLOR' in os.environ else 'auto',
    )


from cmon.parsers import set_client_cli_parser
