...
from cmon.serve.runtimes.monitoring import MonitoringRequestMixin
from cmon.serve.runtimes.worker.request_handling import WorkerRequestHandler
from cmon.types.request.data import DataRequest, Response
from cmon._docarray import docarray_v2
if docarray_v2:
    from cmon.serve.runtimes.helper import _create_pydantic_model_from_schema
...
class HeaderRequestHandler(MonitoringRequestMixin):
    ...
    def __init__(
        ...
        self.name = args.name
        self._deployment_name = os.getenv('CMON_DEPLOYMENT_NAME', 'worker')
    ...
    async def _handle_data_request(
        ...
        for i, worker_result in enumerate(worker_results):
            ...
            if not found and worker_result.status.code == cmon_pb2.StatusProto.SUCCESS:
    ...
    async def endpoint_discovery(self, empty, context) -> cmon_pb2.EndpointsProto:
        ...
        :returns: the response request
        """
        response = cmon_pb2.EndpointsProto()
        try:
            if self.uses_before_address:
                (
    ...
    async def _status(self, empty, context) -> cmon_pb2.CmonInfoProto:
        ...
        infoProto = cmon_pb2.CmonInfoProto()
        version, env_info = get_full_version()
        for k, v in version.items():
            infoProto.cmon[k] = str(v)
