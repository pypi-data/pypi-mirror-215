...
from cmon.constants import __default_endpoint__
from cmon.excepts import BadConfigSource, RuntimeTerminated
from cmon.helper import get_full_version
from cmon.importer import ImportExtensions
from cmon.proto import cmon_pb2
from cmon.serve.executors import BaseExecutor
from cmon.serve.instrumentation import MetricsTimer
from cmon.serve.runtimes.worker.batch_queue import BatchQueue
from cmon.types.request.data import DataRequest
...
class WorkerRequestHandler:
        ...
        **kwargs,
        ...
        if self.metrics_registry:
            with ImportExtensions(
                required=True,
                help_text='You need to install the `prometheus_client` to use the montitoring functionality of cmon',
            ):
    ...
    def _http_fastapi_default_app(self, **kwargs):
        from cmon.serve.runtimes.worker.http_fastapi_app import (  # For Gateway, it works as for head
            get_fastapi_app,
        )
    ...
    def _init_monitoring(
        ...
        if metrics_registry:
            ...
            with ImportExtensions(
                ...
                from cmon.serve.monitoring import _SummaryDeprecated
        ...
        if meter:
            ...
            self._request_size_histogram = meter.create_histogram(
                name='cmon_received_request_bytes',
    ...
    def reduce_requests(requests: List['DataRequest']) -> 'DataRequest':
        ...
        for i, worker_result in enumerate(requests):
            if worker_result.status.code == cmon_pb2.StatusProto.SUCCESS:
    ...
    async def endpoint_discovery(self, empty, context) -> cmon_pb2.EndpointsProto:
        """
        ...
        endpoints_proto = cmon_pb2.EndpointsProto()
        ...
        if docarray_v2:
            ...
            from cmon.serve.runtimes.helper import _create_aux_model_doc_list_to_list
    ...
    async def _status(self, empty, context) -> cmon_pb2.CmonInfoProto:
        """
        Process the the call requested and return the CmonInfo of the Runtime
        ...
        info_proto = cmon_pb2.CmonInfoProto()
        version, env_info = get_full_version()
        for k, v in version.items():
            info_proto.cmon[k] = str(v)
    ...
    ) -> 'cmon_pb2.SnapshotStatusProto':
        _id = str(uuid.uuid4())
        self.logger.debug(f'Generated snapshot id: {_id}')
        return cmon_pb2.SnapshotStatusProto(
            id=cmon_pb2.SnapshotId(value=_id),
            status=cmon_pb2.SnapshotStatusProto.Status.RUNNING,
    ...
    def _create_restore_status(
        ...
        return cmon_pb2.RestoreSnapshotStatusProto(
            id=cmon_pb2.RestoreId(value=_id),
            status=cmon_pb2.RestoreSnapshotStatusProto.Status.RUNNING,
        )

    async def snapshot(self, request, context) -> 'cmon_pb2.SnapshotStatusProto':
        """
    ...
    async def snapshot_status(
        self, request: 'cmon_pb2.SnapshotId', context
    ) -> 'cmon_pb2.SnapshotStatusProto':
        """
        method to start a snapshot process of the Executor
        ...
        if not self._snapshot or (self._snapshot.id.value != request.value):
            return cmon_pb2.SnapshotStatusProto(
                id=cmon_pb2.SnapshotId(value=request.value),
                status=cmon_pb2.SnapshotStatusProto.Status.NOT_FOUND,
            )
            ...
            status = cmon_pb2.SnapshotStatusProto.Status.SUCCEEDED
            if self._did_snapshot_raise_exception.is_set():
                status = cmon_pb2.SnapshotStatusProto.Status.FAILED
            self._did_snapshot_raise_exception = None
            return cmon_pb2.SnapshotStatusProto(
                id=cmon_pb2.SnapshotId(value=request.value),
                status=status,
    ...
    async def restore(self, request: 'cmon_pb2.RestoreSnapshotCommand', context):
        """
    ...
    async def restore_status(
    ...
    ) -> 'cmon_pb2.RestoreSnapshotStatusProto':
        """
        method to start a snapshot process of the Executor
        ...
        if not self._restore or (self._restore.id.value != request.value):
            return cmon_pb2.RestoreSnapshotStatusProto(
                id=cmon_pb2.RestoreId(value=request.value),
                status=cmon_pb2.RestoreSnapshotStatusProto.Status.NOT_FOUND,
            )
            ...
            status = cmon_pb2.RestoreSnapshotStatusProto.Status.SUCCEEDED
            if self._did_restore_raise_exception.is_set():
                status = cmon_pb2.RestoreSnapshotStatusProto.Status.FAILED
            self._did_restore_raise_exception = None
            return cmon_pb2.RestoreSnapshotStatusProto(
                id=cmon_pb2.RestoreId(value=request.value),
                status=status,
