...
from cmon.clients.request import request_generator
from cmon.enums import DataInputType
from cmon.excepts import InternalNetworkError
...
if TYPE_CHECKING:  # pragma: no cover
    ...
    from cmon.serve.runtimes.gateway.streamer import GatewayStreamer
...
def get_fastapi_app(
    ...
    with ImportExtensions(required=True):
        ...
        from cmon.serve.runtimes.gateway.models import (
            CmonEndpointRequestModel,
            CmonRequestModel,
            CmonResponseModel,
        )
    ...
    if not no_debug_endpoints:
        ...
        from cmon._docarray import DocumentArray
        from cmon.proto import cmon_pb2
        from cmon.serve.executors import __dry_run_endpoint__
        from cmon.serve.runtimes.gateway.models import (
            PROTO_TO_PYDANTIC_MODELS
        )
        from cmon.serve.runtimes.gateway.health_model import CmonInfoModel
        from cmon.types.request.status import StatusMessage

        @app.get(
            path='/dry_run',
            summary='Get the readiness of Cmon Flow service, sends an empty DocumentArray to the complete Flow to '
        ...
        async def _flow_health():
            ...
            try:
                ...
                status_message.set_code(cmon_pb2.StatusProto.SUCCESS)
        ...
        @app.get(
            path='/status',
            summary='Get the status of Cmon service',
            response_model=CmonInfoModel,
            tags=['Debug'],
        )
        async def _status():
            """
            Get the status of this Cmon service.

            This is equivalent to running `cmon -vf` from command line.
            ...
            for k, v in env_info.items():
                env_info[k] = str(v)
            return {'cmon': version, 'envs': env_info}

        @app.post(
            path='/post',
            response_model=CmonResponseModel,
            # do not add response_model here, this debug endpoint should not restricts the response model
        )
        async def post(
                body: CmonEndpointRequestModel, response: Response
        ):  # 'response' is a FastAPI response, not a Cmon response
            """
    ...
    def _generate_exception_header(error: InternalNetworkError):
        ...
        from cmon.proto.serializer import DataRequest
    ...
    def expose_executor_endpoint(exec_endpoint, http_path=None, **kwargs):
        ...
        kwargs['response_model'] = kwargs.get(
            'response_model',
            CmonResponseModel,  # use standard response model by default
        )
        ...
        if kwargs['methods'] == ['POST']:
            ...
            async def foo_post(body: CmonRequestModel, response: Response):
                from cmon.enums import DataInputType
            ...
            async def foo_no_post(body: CmonRequestModel):
                from cmon.enums import DataInputType
                bd = body.dict() if body else {'data': None}
                bd['exec_endpoint'] = exec_endpoint
                req_generator_input = bd
