...
from cmon.excepts import ExecutorError
...
from cmon.serve.runtimes.gateway.async_request_response_handling import (
    AsyncRequestResponseHandler,
)
from cmon.serve.runtimes.gateway.graph.topology_graph import TopologyGraph
from cmon.serve.stream import RequestStreamer
...
class GatewayStreamer:
    ...
    def __init__(
        ...
        """
        self.logger = logger or CmonLogger(self.__class__.__name__)
    ...
    async def stream(
        ...
        async for result in self.stream_docs(
            ...
            error = None
            if cmon_pb2.StatusProto.ERROR == result.status.code:
    ...
    def get_streamer():
        ...
        The streamer object is contructed using runtime arguments stored in the `CMON_STREAMER_ARGS` environment variable.
        If this method is used outside a Cmon context (process not controlled/orchestrated by cmon), this method will
        raise an error.
        The streamer object does not have tracing/instrumentation capabilities.

        :return: Returns an instance of `GatewayStreamer`
        """
        if 'CMON_STREAMER_ARGS' in os.environ:
            args_dict = json.loads(os.environ['CMON_STREAMER_ARGS'])
            return GatewayStreamer(**args_dict)
        else:
            raise OSError('CMON_STREAMER_ARGS environment variable is not set')

    @staticmethod
    def _set_env_streamer_args(**kwargs):
        os.environ['CMON_STREAMER_ARGS'] = json.dumps(kwargs)
