...
from cmon.parsers.helper import CastHostAction


def api_to_dict(show_all_args: bool = False):
    """Convert Cmon API to a dict
    :param show_all_args: if set, then hidden args are also exported
    :return: dict
    """
    if show_all_args:
        from cmon.parsers import helper
...
def _export_parser_args(parser_fn, type_as_str: bool = False, **kwargs):
    ...
    from cmon.parsers.helper import _SHOW_ALL_ARGS, CastToIntAction, KVAppendAction
    ...
    for a in parser._actions:
        ...
        if a.dest in random_dest:
            ...
            from cmon.helper import random_identity, random_port
