...
from cmon.constants import __ready_msg__, __stop_msg__, __windows__
from cmon.enums import PodRoleType
from cmon.excepts import RuntimeFailToStart, RuntimeRunForeverEarlyError
from cmon.helper import typename
from cmon.jaml import JAML
from cmon.logging.logger import CmonLogger
from cmon.orchestrate.pods.helper import ConditionalEvent, _get_event
from cmon.parsers.helper import _update_gateway_args
from cmon.serve.executors.run import run, run_raft
from cmon.constants import RAFT_TO_EXECUTOR_PORT
...
class BasePod(ABC):
    ...
    def __init__(self, args: 'argparse.Namespace'):
        ...
        self._envs = {'CMON_DEPLOYMENT_NAME': self.name}
        if self.args.quiet:
            self._envs['CMON_LOG_CONFIG'] = 'QUIET'
        if self.args.env:
...
class Pod(BasePod):
    ...
    def _get_runtime_cls(self):
        from cmon.orchestrate.pods.helper import update_runtime_cls
