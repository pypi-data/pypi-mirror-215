...
from cmon.constants import __args_executor_init__, __cache_path__, __default_endpoint__
from cmon.enums import BetterEnum
...
from cmon.jaml import JAML, JAMLCompatible, env_var_regex, internal_var_regex
from cmon.logging.logger import CmonLogger
from cmon.serve.executors.decorators import (
    _init_requests_by_class,
    avoid_concurrent_lock_cls,
)
from cmon.serve.executors.metas import get_executor_taboo
from cmon.serve.helper import (
...
__dry_run_endpoint__ = '_cmon_dry_run_'
...
class BaseExecutor(JAMLCompatible, metaclass=ExecutorType):
    ...
    :class:`cmon.Executor` as an alias for this class.

    EXAMPLE USAGE

    .. code-block:: python

        from cmon import Executor, requests, Flow
    ...
    Any executor inherited from :class:`BaseExecutor` always has the **meta** defined in :mod:`cmon.executors.metas.defaults`.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.logger = CmonLogger(self.__class__.__name__, **vars(self.runtime_args))
    ...
    def _init_monitoring(self):
        ...
        if self.meter:
            self._process_request_histogram = self.meter.create_histogram(
                name='cmon_process_request_seconds',
                description='Time spent when calling the executor request method',
            )
            self._histogram_buffer = {
                'cmon_process_request_seconds': self._process_request_histogram
            }
        else:
    ...
    def _add_requests(self, _requests: Optional[Dict]):
        if _requests:
            ...
            for endpoint, func in _requests.items():
                ...
                elif typename(_func) == 'cmon.executors.decorators.FunctionMapper':
    ...
    def _add_metas(self, _metas: Optional[Dict]):
        from cmon.serve.executors.metas import get_default_metas
    ...
    def from_hub(
        ...
        **kwargs,
        ...
        :param uri: a hub Executor scheme starts with `cmonhub://`
        ...
        :param kwargs: other kwargs accepted by the CLI ``cmon hub pull``
        ...
        .. code-block:: python
            ...
            executor = Executor.from_hub(
                uri='cmonhub://CLIPImageEncoder', install_requirements=True
            )
    ...
    def monitor(
        ...
        if self._histogram_buffer and not _histogram:
            _histogram = self.meter.create_histogram(
                name=f'cmon_{name}', description=documentation
            )
