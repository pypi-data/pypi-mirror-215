...
from cmon.excepts import PortAlreadyUsed, RuntimeTerminated
from cmon.helper import ArgNamespace, is_port_free, random_ports, send_telemetry_event
from cmon.logging.logger import CmonLogger
from cmon.parsers import set_gateway_parser
from cmon.parsers.helper import _set_gateway_uses

# Keep these imports even if not used, since YAML parser needs to find them in imported modules
from cmon.serve.runtimes.gateway.composite import CompositeGateway
from cmon.serve.runtimes.gateway.gateway import BaseGateway
from cmon.serve.runtimes.gateway.grpc import GRPCGateway
from cmon.serve.runtimes.gateway.http import HTTPGateway
from cmon.serve.runtimes.gateway.websocket import WebSocketGateway
...
class AsyncNewLoopRuntime:
    ...
    def __init__(
        ...
        self.logger = CmonLogger(self.name, **vars(self.args))
    ...
    def _get_server(self):
        ...
        if self.req_handler_cls.__name__ == 'GatewayRequestHandler':
            ...
            from cmon.serve.runtimes.servers.grpc import GRPCServer
            ...
            from cmon.serve.runtimes.servers.http import HTTPServer  # we need a concrete implementation of this
            ...
            from cmon.serve.runtimes.servers.websocket import \
            ...
            from cmon.serve.runtimes.servers.composite import \
