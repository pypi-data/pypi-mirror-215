...
from cmon.constants import __resources_path__, __uptime__, __windows__
from cmon.enums import LogVerbosity
from cmon.jaml import JAML
from cmon.logging import formatter
...
class CmonLogger:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if quiet or os.getenv('CMON_LOG_CONFIG', None) == 'QUIET':
            log_config = 'quiet'

        if not name:
            name = os.getenv('CMON_DEPLOYMENT_NAME', context)
    ...
    def add_handlers(self, config_path: Optional[str] = None, **kwargs):
        ...
        if 'CMON_LOG_LEVEL' in os.environ:
            verbose_level = LogVerbosity.from_string(os.environ['CMON_LOG_LEVEL'])
