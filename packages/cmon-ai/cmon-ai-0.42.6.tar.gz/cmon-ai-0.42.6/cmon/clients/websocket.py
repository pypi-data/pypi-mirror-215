from cmon.clients.base.websocket import WebSocketBaseClient
from cmon.clients.mixin import (
...
class WebSocketClient(WebSocketBaseClient, PostMixin, ProfileMixin, HealthCheckMixin):
    ...
    Instantiate this class through the :meth:`cmon.Client` convenience method.
...
class AsyncWebSocketClient(
    ...
    the integration settings, where Cmon/Flow/Client is NOT the main logic, but rather served as a part of other program.
    In this case, users often do not want to let Cmon control the ``asyncio.eventloop``. On contrary, :class:`Client`
