...
from cmon.excepts import BadConfigSource
from cmon.helper import is_yaml_filepath
from cmon.importer import PathImporter


class CmonConstructor(FullConstructor):
...
class CmonResolver(Resolver):
    """Remove `on|On|ON` as bool resolver."""

    pass


class CmonLoader(Reader, Scanner, Parser, Composer, CmonConstructor, CmonResolver):
    """
    The Cmon loader which should be able to load YAML safely.
    ...
    def __init__(self, stream, runtime_args=None):
        ...
        CmonConstructor.__init__(self)
        CmonResolver.__init__(self)
        self.runtime_args = runtime_args


def get_cmon_loader_with_runtime(runtime_args: Optional[Dict[str, Any]] = None):
    """Create a CmonLoader init function which already stored the runtime_args in the init function, usefully for
    `yaml.load(stream,loader=CmonLoader)` which needs a class with a init function with only one parameter

    :param runtime_args: Optional runtime_args to be directly passed without being parsed into a yaml config
    :return: A function that initialize a CmonLoader with the runtime_args stored within the function

    """

    def _get_loader(stream):
        return CmonLoader(stream, runtime_args)

    return _get_loader


# remove on|On|ON resolver
CmonResolver.yaml_implicit_resolvers.pop('o')
CmonResolver.yaml_implicit_resolvers.pop('O')
