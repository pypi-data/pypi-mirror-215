...
class RabbitMonitor:
    ...
    def get_idle_time_in_secs(self, queue_name, vhost):
        data = self._get_queue_info(queue_name, vhost)
        if 'idle_since' in data:
            return self._get_seconds_since_datetime(data['idle_since'])
        else:
            return 0

    @staticmethod
    def _get_seconds_since_datetime(datetime_str):
