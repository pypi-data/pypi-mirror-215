...
class ConfigurationGenerator():
    ...
    def generate_configuration(self, **kwargs):
        ...
        start_time = time.time()
        explainability_configuration, explain_perturbation_df = self.__generate_explainability_config(
        ...
        self.config_obj.explain_perturbation_df = explain_perturbation_df
    ...
    def __generate_explainability_config(self, spark_df, constraint_set, configuration):
        ...
        if self.enable_explainability:
            ...
            perturbations = Perturbations(
                training_stats=explainability_configuration, problem_type=self.model_type)
    ...
    def __create_archive(self, config_obj):
        ...
        if self.enable_explainability:
            explainability_perturbations_filename = "explainability_perturbations.tar.gz"
            with tarfile.open(explainability_perturbations_filename, mode="w:gz") as perturbation_tar:
                if config_obj.explain_perturbation_df is not None:
                    explain_perturbation = BytesIO(
        ...
        with tarfile.open(configuration_archive_name, mode="w:gz") as configuration_tar:
            ...
            # Add explainability perturbation archive to tar
            if self.enable_explainability:
                configuration_tar.add(explainability_perturbations_filename)
            if self.enable_drift:
        ...
        if self.enable_explainability:
            os.remove(explainability_perturbations_filename)
