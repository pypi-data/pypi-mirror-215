...
def get_config(config_file_name):
    ...
    :return:
    """
    # is there a config in the current working directory?
    cwd_path = os.getcwd()
    config_file_path = os.path.join(cwd_path, config_file_name)
    if not os.path.exists(config_file_path):
        # load the default config file
        file_path = pathlib.Path(__file__).parent
        config_file_path = os.path.join(file_path, config_file_name)
...
def main():
    ...
    series_list = parse_files(file_path, config)
    if len(series_list) > 0:
        for series in series_list:
            print(get_statistics(series))

        # use percentiles over specific thresholds ...
        series_list2 = filter_by_percentile(series_list, config.get("percentile", 99))
