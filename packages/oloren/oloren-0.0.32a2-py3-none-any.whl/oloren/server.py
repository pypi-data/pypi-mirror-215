...
async def post_log_message(dispatcher_url, myUuid, progressId, level, message):
    print("POSTING LOG MESSAGE to PROGRESS")
    response = await asyncio.get_event_loop().run_in_executor(None, requests.post, f"{dispatcher_url}/node_progress",
        {
...
def log_message(dispatcher_url, myUuid, progressId, level, message):
    asyncio.create_task(post_log_message(dispatcher_url, myUuid, progressId, level, message))
