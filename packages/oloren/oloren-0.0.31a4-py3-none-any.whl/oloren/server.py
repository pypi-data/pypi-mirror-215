...
def log_message(dispatcher_url, myUuid, progressId, level, message):
    ...
    response = requests.post(f"{dispatcher_url}/node_progress", data=form_data)
...
def execute_function(dispatcher_url_, body, FUNCTION_NAME):
    ...
    socket.emit("extensionregister", data={"id": body["id"]}, callback=on_extensionregister_response)
    socket.wait()
    socket.disconnect()
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            outputs = FUNCTIONS[FUNCTION_NAME][0](*inputs, log_message=get_log_message_function(dispatcher_url_, client_info["uuid"]))
