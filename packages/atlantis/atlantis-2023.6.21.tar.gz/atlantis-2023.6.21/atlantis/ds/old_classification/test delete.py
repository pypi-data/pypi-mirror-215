...
class Classifier:
	def __init__(
		...
		if classifier is None or classifier == 'random_forest':
			classifier = RandomForestClassifier(
				labelCol=self._label_column,
	...
	def fit(self, data):
		...
		self._classes = transformed_data.select(self._label_column).distinct().rdd.flatMap(lambda x: x).collect()
	...
	def measure_performance(self, data, label=None):
		...
		if label is None:
			self._performance = pd.DataFrame.from_records(
				[self._measure_performance_of_counts(data=result, label=l) for l in self._classes])
		else:
			self._performance = self._measure_performance_of_counts(data=result, label=label)
