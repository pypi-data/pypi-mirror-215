...
class SuperuserSessionLogDownloadRecord(TypedObject):
    ...
    *(extends* :py:class:`delphixpy.web.vo.TypedObject` *)* Represents a
    ...
    def to_dict(self, dirty=False, belongs_to_parent=False):
        ...
        if "user_ip_address" == "type" or (self.user_ip_address is not self.__undef__ and (not (dirty and not self._user_ip_address[1]) or isinstance(self.user_ip_address, list) or belongs_to_parent)):
            dct["userIpAddress"] = dictify(self.user_ip_address)
        if "log_name" == "type" or (self.log_name is not self.__undef__ and (not (dirty and not self._log_name[1]) or isinstance(self.log_name, list) or belongs_to_parent)):
            dct["logName"] = dictify(self.log_name)
        if "succeeded" == "type" or (self.succeeded is not self.__undef__ and (not (dirty and not self._succeeded[1]) or isinstance(self.succeeded, list) or belongs_to_parent)):
