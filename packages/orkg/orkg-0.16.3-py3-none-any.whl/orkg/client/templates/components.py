...
from orkg.client.templates.utils import pre_process_string, check_against_keywords, map_type_to_pythonic_type
...
DataFrame = Union[pd.DataFrame, Tuple[pd.DataFrame, str], OID, List[pd.DataFrame], List[Tuple[pd.DataFrame, str]], List[OID]]
...
class TemplateComponent:
    ...
    def __init__(self, orkg_client, component_id: str):
        ...
        value_class = list(
            filter(lambda x: x['predicate']['id'] in ['sh:class', 'sh:datatype'], component_statements)
        )
        ...
        max_cardinality = list(
            filter(lambda x: x['predicate']['id'] == 'sh:maxCount', component_statements)
        )
        self.max_cardinality = None if len(max_cardinality) == 0 else int(max_cardinality[0]['object']['label'])
        min_cardinality = list(
            filter(lambda x: x['predicate']['id'] == 'sh:minCount', component_statements)
        )
        self.min_cardinality = None if len(min_cardinality) == 0 else int(min_cardinality[0]['object']['label'])
...
class Template(object):
    ...
    def __init__(self, orkg_client, template_id: str):
        ...
        components = filter(lambda x: x['predicate']['id'] == 'sh:property', template_statements)
        for component in components:
            self.components.append(TemplateComponent(orkg_client, component['object']['id']))
        # Set template class
        self.template_class = list(
            filter(lambda x: x['predicate']['id'] == 'sh:targetClass', template_statements)
        )[0]['object']['id']
        # Set template info
        ...
        strict_statement = list(filter(lambda x: x['predicate']['id'] == 'sh:closed', template_statements))
        self.is_strict = len(strict_statement) > 0 and strict_statement[0]['object']['label'].lower() == 'true'
