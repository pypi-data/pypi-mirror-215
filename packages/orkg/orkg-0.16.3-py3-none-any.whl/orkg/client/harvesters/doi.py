        ...
        for key, value in contribution_json.items():
            if key == "@id":
                ...
                if isinstance(value, List) and all(isinstance(item, str) for item in value):
                    resulting_object["values"][orkg_predicate_id] = [{"text": str(item)} for item in value]
...
def harvest(orkg_client: Any, doi: Optional[str], orkg_rf: Union[str, OID], directory: Optional[str]) -> OrkgResponse:
    ...
    if directory is not None:
        ...
        if doi is None and not os.path.isfile(os.path.join(directory, 'doi.json')):
            raise ValueError(f'The directory {directory} does not contain the file doi.json.')
    ...
    if directory is not None:
        files = [f for f in os.listdir(directory) if os.path.isfile(os.path.join(directory, f)) and f != 'doi.json']
