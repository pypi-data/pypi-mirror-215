...
class Core():
    ...
    def om_open(self,ws):
        print('连接虚仿端成功')
        self.sendmsg(json.dumps({'eventid':0, 'type':'checkSdk','data':Version}))
    ...
    def doAwait(self,data:dict):
        """
        执行等待方法
        :param data:
        """
        result=self.loop.run_until_complete(self.send(data))
    ...
    async def send(self, data:dict):
        ...
        id=self.eventId
        data['eventid']=id
        self.asyncEvects[id] = asyncio.Event()
        self.sendmsg(json.dumps(data))
        print("开始等待"+str(id))
        await Core.asyncEvects[id].wait()
        obj= self.resultEvents[id]
        del self.resultEvents[id]
        return obj
