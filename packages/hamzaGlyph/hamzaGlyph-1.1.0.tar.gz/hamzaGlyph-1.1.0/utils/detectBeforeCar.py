...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                # Extract the ROI
                roi = im[top_left[1]:bottom_right[1], top_left[0]:bottom_right[0]] 
   ...
   # Save maximum bounding box coordinates to file
    #if max_bbox is not None:
     #   os.makedirs(os.path.dirname(f'{txt_path}_max.txt'), exist_ok=True)  # create directory if it doesn't exist
      #  with open(f'{txt_path}_max.txt', 'w') as f:
       #     f.write(','.join(str(x) for x in max_bbox))
