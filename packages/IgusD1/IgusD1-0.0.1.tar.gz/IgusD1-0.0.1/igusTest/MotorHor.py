...
def isRefHorizontal(ser):
    dataList = []
    data = str(igusTest.initialisierung.readData("1A", ser))
    dataList = data.split('\\')

    igusTest.initialisierung.error(dataList, ser)
    if (dataList[3] != "xff"):
        ref_horizontal(ser)
...
def horIsActive(ser):
    ...
    while (isActive == True):
        ...
        data = str(igusTest.initialisierung.readData("1A", ser))  # new values to variable
        dataList = data.split('\\')
        if dataList[7] == "xff":
            isActiv = True
        elif dataList[7] != "xff":
...
def pos1_horizontal(ser):
    ...
    horIsActive(ser)
    print("MotorHor Pos1: " + str(igusTest.initialisierung.readData("1A", ser)))
...
def pos2_horizontal(ser):
    ...
    ser.write(bytes.fromhex("71"))
    horIsActive(ser)
    print("MotorHor Pos2: " + str(igusTest.initialisierung.readData("1A", ser)))
...
def pos3_horizontal(ser):
    ...
    ser.write(bytes.fromhex("71"))
    horIsActive(ser)
    print("MotorHor Pos3: " + str(igusTest.initialisierung.readData("1A", ser)))