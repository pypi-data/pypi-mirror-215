...
def model_ab_test(test_ds:Dataset, modelA:StringElement , modelB:StringElement , type:ModelABTestTypeElement, aggregation_level:AggregationLevelElement, rules:ModelABTestRules, gt:Optional[StringElement]=StringElement(""), filter:Optional[StringElement]=StringElement("")):
    retries = 0
    while retries < MAX_RETRIES:
        try:
            initialize(test_ds, modelA, modelB, type, aggregation_level, rules, gt, filter)
...
def initialize(test_ds:Dataset, modelA:StringElement , modelB:StringElement , type:ModelABTestTypeElement, aggregation_level:AggregationLevelElement, rules:ModelABTestRules, gt:Optional[StringElement]=StringElement(""), filter:Optional[StringElement]=StringElement("")):
    validation(test_ds, modelA, modelB, type, aggregation_level, rules, gt)
    create_ab_test(test_ds, modelA, modelB, type, aggregation_level, rules, gt, filter)
    return True

def create_ab_test(test_ds:Dataset, modelA:StringElement , modelB:StringElement , type:ModelABTestTypeElement, aggregation_level:AggregationLevelElement, rules:ModelABTestRules, gt:Optional[StringElement]=StringElement(""), filter:Optional[StringElement]=StringElement("")):
    payload = {
        "datasetId": test_ds.dataset_id,
        "experimentId": test_ds.test_session.experiment_id,
        "name": test_ds.test_session.run_name,
        "filter": filter.get(),
        "modelA": modelA.get(),
        "modelB": modelB.get(),
        "gt":gt.get(),
...
def validation(test_ds: Dataset, modelA: StringElement, modelB: StringElement, type: ModelABTestTypeElement, aggregation_level: AggregationLevelElement, rules: ModelABTestRules, gt: Optional[StringElement] = StringElement("")):
