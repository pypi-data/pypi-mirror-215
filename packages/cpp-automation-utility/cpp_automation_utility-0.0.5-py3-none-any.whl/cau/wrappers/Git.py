...
class Git():
    ...
    def changed_files(self) -> DiffCollection:
        """
        Interrogates git to get a list of changed files between working directory
        and the remote main line branch

        Returns:
            DiffCollection: changed file commits
        """
        remote: git.Remote = self._repo.remote(name=str(self.main_branch.parent))
        main: git.RemoteReference = [ref for ref in remote.refs if str(self.main_branch) == ref.name][0]
        ...
        changes = list(set(remote_head.diff("HEAD") + self._repo.index.diff("HEAD") + self._repo.index.diff(None)))
        return changes
