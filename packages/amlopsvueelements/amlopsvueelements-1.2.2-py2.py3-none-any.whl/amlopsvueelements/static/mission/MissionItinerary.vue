  ...
  <div :class="$style['mission-itinerary']">
    <div :class="$style['mission-itinerary__items']">
      <div
        ...
        :key="index"
      >
        <UFormWrapper :is-loading="isLoading || isCancelingMissionLeg" :class="$style['mission-itinerary__wrapper']">
          <template #header>
            <div :class="$style['mission-itinerary__header']">
              ...
              <UButton
                ...
                v-if="leg.sequence_id !== 1 && leg.sequence_id !== formData.legs?.length"
    ...
    <div class="my-auto flex flex-col w-1/3 gap-[28px]">
      <template v-for="(leg, index) in formData.legs as IMissionLeg[]" :key="index">
        <div v-if="leg?.servicing" :class="$style['mission-itinerary__aml']">
          <div :key="index" :class="$style['mission-itinerary__item']">
            <UFormWrapper
              ...
              class="relative"
            >
              <template #header> Fueling {{ leg?.sequence_id }}</template>
              <template #content>
...
watch(()=> formData.value.legs?.[0].servicing.services.length, (newValue,oldValue)=>{
...
.mission-itinerary {
  ...
  .add-new-mission {
    ...
    &__sign {
      @apply w-5 h-5 text-grey-800 flex text-[0.96875rem] justify-center rounded-full bg-white mx-auto text-center relative;
    }
  }
}
</style>
