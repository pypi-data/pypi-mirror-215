  ...
  <div :class="[$style['ops-aml-turnaround-wrapper']]">
    <div :class="[$style['ops-aml-turnaround__content']]">
      <div
          ...
          @click.stop.prevent.capture="onHandleAmlServicing"
      >
        <UCheckboxWrapper :label-text="`AML Servicing at ${arrivalAirportShortName}`"
      ...
      <div v-if="missionFormModel.legs[legIndex]?.servicing">
          ...
          <div class="flex font-medium text-[1.25rem] text-grey-1000">
              ...
              "
                      ...
                      />
                      <div v-if="missionFormModel.legs[legIndex].servicing.services?.length && !searchPristQuery"
                  ...
                  <div
                          v-if="missionFormModel.legs[legIndex].servicing.services"
        ...
        <div v-else class="h-[360px]">
            <div :class="[$style['ops-aml-turnaround__error']]">
...
import {FuelRequiredTypes, missionDefaultFormModel, missionLegServicingDefaults} from '@/constants/mission.constants'
...
import type {IService, IServiceAvailability} from '@/types/mission/mission-reference.types'
...
} = useFetch<IService[], (locationId: number, organisationId?: number) => void>(async (locationId: number, organisationId?: number) => {
  ...
  return (
      (airportLocations.value &&
          airportLocations.value?.find((airport: IAirport) => {
            return airport.id === computedLegInfo.value.arrival_location ? airport : ''
          })?.tiny_repr) ||
      ''
  )
})
...
const onToggleAmlServicing = (flag: boolean) => {
  ...
  if (flag) return missionFormModel.value.legs[props.legIndex].servicing = missionDefaultFormModel()
...
const onCancelServicing = async () => {
  const {isConfirmed} = await window.Swal({
    title: 'Confirmation',
    text: `Please confirm that you wish cancel the servicing & fueling arrangements at ${arrivalAirportShortName.value} for legs ${
        props.legIndex + 1
    } & ${props.legIndex + 2}`,
    icon: 'info',
...
const findServiceNameById = (id: string): { name: string, arrival: boolean, departure: boolean } => {
  const res = (serviceOptions.value as IService[])?.find((option: IService) => option.id === id)
  const availability = res?.attributes?.availability_bool?.find((item) => item.airport === computedLegInfo.value.arrival_location)
  return {
      arrival: true,
      departure: true,
...
watch(
    ...
    async ([locationId, organisationId], [oldLocation]) => {
          if(locationId !== oldLocation) missionFormModel.value.legs[props.legIndex].servicing.services = []
...
.ops-aml-turnaround {
  ...
  &__content {
    ...
    &__delete-icon {
      ...
      filter: invert(23%) sepia(85%) saturate(2552%) hue-rotate(330deg) brightness(87%) contrast(103%);
    }
  }
    &__error {
