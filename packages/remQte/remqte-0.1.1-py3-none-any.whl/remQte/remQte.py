...
co = ConfigParser()
initv = "%s/tvs.ini"%os.path.dirname(__file__) 
inich = "%s/channels.ini"%os.path.dirname(__file__)
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if len(ini_tvs.sections()) == 0:
            uic.loadUi(os.path.dirname(__file__) + ("/qtui/main_start.ui"), self)
            self.btnscan.clicked.connect(self.network)
        else:
            stv = stvws()
            self.tvup = True if self.nett.png(stv.default) == 0 else False
            uic.loadUi(os.path.dirname(__file__) + ("/qtui/remote.ui"), self)
    ...
    def start(self):
        if len(ini_tvs.sections())>0:
            self.remote()
    def importch(self):
        self.w = uic.loadUi(os.path.dirname(__file__) + ("/qtui/importcsv_s1.ui"))
        self.w.show()
    ...
    def open_filedialog(self):
        ...
        uic.loadUi(os.path.dirname(__file__) + ("/qtui/importcsv_s2.ui"), self.w)
...
#        self.pushButton_2.clicked.connect(self.step2)
    ...
    #    importapp.exec()
    def network(self):
        uic.loadUi(os.path.dirname(__file__) + ("/qtui/main_scan_choose_nets.ui"), self)
    ...
    def thread_complete(self):
        uic.loadUi(os.path.dirname(__file__) + ("/qtui/main_scan_nets.ui"), self)
        tb = self.tableWidget
        it = QTableWidgetItem
        tb.setColumnCount(3)
...
#           tb.setItem(o,2, it(itm['type']))
    ...
    def remote(s):
        uic.loadUi(os.path.dirname(__file__) + ("/qtui/remote.ui"),s)
