...
def get_line(
    center_x: float, center_y: float, length: float, orientation: VisLineOrientation
):
    if orientation == VisLineOrientation.HORIZONTAL:
...
class VisLineDetector:
    def __init__(
        ...
        self.hor_kernel = np.ones((KERNEL_WIDTH, KERNEL_HEIGHT), np.uint8) / (
            KERNEL_WIDTH * KERNEL_HEIGHT
        )
        self.ver_kernel = np.ones((KERNEL_HEIGHT, KERNEL_WIDTH), np.uint8) / (
            KERNEL_WIDTH * KERNEL_HEIGHT
        )
