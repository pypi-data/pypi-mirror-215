...
class PageCreator:
    ...
    def get_tables(self, words: list[Word], lines: list[Line]) -> list[Table]:
        try:
    ...
    def create_page(self) -> Page:
        ...
        tables = self.get_tables(words, lines)
        table_lines = list(itertools.chain(*(table.get_all(Line) for table in tables)))
        lines = [line for line in lines if line not in table_lines]
        ...
        borderless_table_lines = list(
            itertools.chain(*(table.get_all(Line) for table in borderless_tables))
        )
        lines = [line for line in lines if line not in borderless_table_lines]
