...
def write_to_excel(ps_project_name, ps_designer, ps_date, ps_note,
    ...
    trq, v_at_rd, fb_max, d_pin, l_pin, w_frk, d_frk, w_bush, th4d_max, link_ok_flag, intrf_c_fork_flag, intrf_f_bore_flag, intrf_g_fork_flag, fg, fc, fa, j, 
    th2d_at_rd, th3d_at_rd, th4d_at_rd, th5d_at_rd, th6d_at_rd, thd_j, thd_cb, thd_bj, thd_jg, thd_a, 
    stk, drv_mode, d_max, rd_act,
    sc_bush_all, sc_frk_all, st_frk_all, rat_cd_d_frk, rat_cd_f, fra_rev):
    ...
    ws5_row_intrf_f_bore_flag = 12
    ws5_row_intrf_g_fork_flag = 13
    ws5_row_fg = 14
    ws5_row_fc = 15
    ws5_row_fa = 16
    ws5_row_j = 17
    ws5_row_th2d_at_rd = 18
    ws5_row_th3d_at_rd = 19
    ws5_row_th4d_at_rd = 20
    ws5_row_th5d_at_rd = 21
    ws5_row_th6d_at_rd = 22
    ws5_row_thd_j = 23
    ws5_row_thd_cb = 24
    ws5_row_thd_bj = 25
    ws5_row_thd_jg = 26
    ...
    ws4.write(ws4_row_g, 0, "g (lower conrod)")
    ...
    ws5.write(ws5_row_trq_gear, 0, "gear axis torque (Nm)")
    ws5.write(ws5_row_vrd, 0, "slide vel at RD (mm/s)")
    ws5.write(ws5_row_f_rkr, 0, "conrod force (ton)")
    ws5.write(ws5_row_d_pin, 0, "dia of main load bearing pins (mm)")
    ws5.write(ws5_row_l_pin, 0, "length of main load bearing pins as input by user (mm)")
    ...
    ws5.write(ws5_row_intrf_f_bore_flag, 0, "Link f bore too close?")
    ws5.write(ws5_row_intrf_g_fork_flag, 0, "Link g interference with fork?")

    ws5.write(ws5_row_fg, 0, "Force on lower conrod (ton)")
    ws5.write(ws5_row_fc, 0, "Force on (upper) rocker link (ton)")
    ws5.write(ws5_row_fa, 0, "Force on horizontal conrod bigend (ton)")
    ws5.write(ws5_row_j, 0, "Ternary link third side j (mm)")
    ...
    ws5.write(ws5_row_thd_j, 0, "Angle of j at RD (deg)")
    ws5.write(ws5_row_thd_cb, 0, "Angle bw link c and b at RD (deg)")
    ws5.write(ws5_row_thd_bj, 0, "Angle bw ternary link length b and j (deg)")
    ws5.write(ws5_row_thd_jg, 0, "Angle bw link j and g at RD (deg)")
    ws5.write(ws5_row_thd_a, 0, "Angle of force direction at horizontal conrod big end (deg)")
    ...
    ws6.write(ws6_row_rat_cd_f, 0, "min ratio of ternary vertical link f to ternary link pin dia")
    ...
    ws5.write(ws5_row_l_pin, 1, l_pin)
    ...
    ws5.write(ws5_row_intrf_f_bore_flag, 1, intrf_f_bore_flag)
    ws5.write(ws5_row_intrf_g_fork_flag, 1, intrf_g_fork_flag)
    ws5.write(ws5_row_fg, 1, fg)
    ws5.write(ws5_row_fc, 1, fc)
    ws5.write(ws5_row_fa, 1, fa)
    ws5.write(ws5_row_j, 1, j)
    ...
    ws5.write(ws5_row_thd_j, 1, thd_j)
    ws5.write(ws5_row_thd_cb, 1, thd_cb)
    ws5.write(ws5_row_thd_bj, 1, thd_bj)
    ws5.write(ws5_row_thd_jg, 1, thd_jg)
    ws5.write(ws5_row_thd_a, 1, thd_a)
    ...
    ws6.write(ws6_row_rat_cd_f, 1, rat_cd_f)
