...
def get_graphs(th2d_lst, fbos_lst, v_lst, acc_lst, th2d_tt360str_lst, 
	fbos_tt_lst, v_tt_lst, fbos_mb_lst, v_mb_lst, f_mb_lst, fc_mb_lst):
	...
	f_mb_lst: list
	    ...
	    at BDC.
	fc_mb_lst: list
	    List of conrod force in ton starting from downstroke mid and ending
	    at BDC.
	...
	graph_13, = axs[1, 3].plot(fbos_mb_lst, fc_mb_lst, 'tab:red')
	# axs[1, 2].legend(['force'], loc="upper right")
	axs[1, 3].grid(visible=None, which='major', axis='both')
	axs[1, 3].title.set_text('rocker force vs. fbos')
	axs[1, 3].set_xlabel('fbos (mm)')
	axs[1, 3].set_ylabel('total rocker force (ton)')
