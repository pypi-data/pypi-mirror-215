...
class MayLinkSizer:
	"""
	A class for sizing of maylink mechanism items
	...
	...
	pf : float
		press force in ton
	f : float
		length vertical ter link side in mm
	g : float
	...
	get_intrf_c_fork_flag():
		flag to check interf of fork grooves of rocker link c
	get_intrf_f_bore_flag():
		flag to check mearging condition of 2 bores of link f
	get_intrf_g_fork_flag():
	...
	def __init__(self, c, pf, f, g, l_pin, fra_rev, th4d_lst):
		sc_bush_all = 80  # MPa, allowable contact stress in bronze bush for main load
		sc_frk_all = 40  # MPa, allowable contact stress in link fork area (Class 4 forging including SCF)
		st_frk_all = 120  # MPa, allowable tensile stress in link fork area (Class 4 forging including SCF)

		rat_cd_d_frk = 1.2  # ratio of link center distance to fork dia (link od)
		rat_cd_f = 1.1  # ratio of ternary link vertical link f to ternary link pin dia

		pf_rev = fra_rev * pf  # reverse load in ton
		self.th4d_max = get_th_rkr_max(th4d_lst)
		th4_max = self.th4d_max * math.pi / 180
		self.d_pin = (pf + 2 * pf_rev) * 10000 / (sc_bush_all * l_pin * math.cos(th4_max))
		...
		if not self.intrf_c_fork_flag and not self.intrf_f_bore_flag and not self.intrf_g_fork_flag:
			self.link_ok_flag = True

		# print("c / self.d_frk", c / self.d_frk)
		# print("f / self.d_pin", f / self.d_pin)
		# print("g / self.d_frk", g / self.d_frk)
	...
	def get_intrf_f_bore_flag(self):
		"""
        Returns
        -------
        boolean
        flag to check mearging condition of 2 bores of link f
        """
		return self.intrf_f_bore_flag  # True if interference
	...
	def get_setting_dic(self):
        ...
        contains all setting data like allowable stresses etc
        """
		return self.setting_dic  # True if interference
