...
import math
import pandas as pd
from customs import custom_inputs
...
def get_custom_dimensions():
    """
    Retrieve custom dimensions and metrics from customs.py
    ...
    Returns:
        ...
        custom_metrics
    """
   #call custom_input function to get custom metrics and dimensions
    custom_dimensions, custom_metrics = custom_inputs()
...
def run_query(service: object,
    ...
    # Convert dimension names to 'ga:dimensionX' format if applicable
    custom_dimensions, custom_metrics = get_custom_dimensions()
    if 'dimensions' in payload:
        payload['dimensions'] = [custom_dimensions[dim] if dim in custom_dimensions else dim for dim in payload['dimensions']]
...
"""Try this to test the run_query function
from query import run_query, get_custom_dimensions

custom_dimensions, custom_metrics = get_custom_dimensions()
print(f"Custom Dimensions: {custom_dimensions}")
print(f"Custom Metrics: {custom_metrics}")

service = "votre_service_Google_Analytics"
view_id = "votre_view_id"
payload = {"votre": "payload"}

# Nous utilisons les valeurs par défaut pour output et verbose
result = run_query(service, view_id, payload)
print(result)
"""
