...
class EUDBag:
    def __init__(self, capacity):
        ...
        self.loopvar = {name: c.EUDVariable() for name in args if not name.startswith("__")}
    ...
    def add(self, *args, **kwargs):
        if len(args) > len(self.non_default) + len(self.default):
            raise TypeError(
                f"{self.__class__.__name__}.add takes from {len(self.non_default)} to {len(self.non_default) + len(self.default)} positional arguments but {len(args)} were given"
            )
        if len(args) < len(self.non_default):
            ...
            if missing_args:
                raise TypeError(
                    f"{self.__class__.__name__}.add missing {len(missing_args)} required positional arguments: {missing_args}"
                )
        for arg in self.non_default[: len(args)]:
            if arg in kwargs:
                raise TypeError(
                    f"{self.__class__.__name__}.add got multiple values for argument {arg}"
                )
