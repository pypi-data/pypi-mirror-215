...
def __read_file_text(file):
	return open(file, "r", encoding = __encoding).read()
...
class ChadExtractor:
	...
	async def __request_get(self, context, url):
		...
		try:
			...
			tmp["response"] = body.decode(self.__encoding)
			if self.__debug:
				self.__print_debug(response.status, url)
		except PlaywrightTimeoutError:
			pass
...
class Validate:
	...
	def __basic(self):
		...
		print("Chad Extractor v3.9 ( github.com/ivan-sincek/chad )")
		print("")
