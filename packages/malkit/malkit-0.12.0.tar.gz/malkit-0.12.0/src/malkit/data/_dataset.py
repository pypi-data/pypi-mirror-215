...
class ClassifiedDataset:
    def __init__(
        ...
        *,
        ...
        self._index_to_class = sorted(p.name for p in root.iterdir() if p.is_dir())
        self._class_to_index = {c: i for i, c in enumerate(self._index_to_class)}

        if num_classes is not None and len(self._class_to_index) != num_classes:
            raise RuntimeError("num_classes check failure, the root directory may be dirty")
    ...
    def load_X_y_parallel(self, *, n_jobs: Optional[int] = None) -> Tuple[NDArray, NDArray]:
        ...
        for index in range(len(self)):
            ...
            y.append(target)
        X = self.loader.parallel(sample_paths, n_jobs=n_jobs)
        return X, np.stack(y)
    ...
    def __repr__(self) -> str:
        ...
        s += f"    Number of classes: {self.num_classes}\n"
        return s
