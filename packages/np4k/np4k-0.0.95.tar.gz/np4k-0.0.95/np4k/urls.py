...
def valid_url(url: str, verbose: bool = False, test: bool = False, source_domain: Optional[str] = None) -> bool:
    """
    Is this URL a valid news-article url?

    Perform a regex check on an absolute url.

    1. perform a few basic checks like making sure the format of the url
    is right, (scheme, domain, tld).

    2. make sure that the url isn't some static resource, check the
    file type.

    3. search of a YYYY/MM/DD pattern in the url. News sites
    ...
    # CHECK 2: siphon out the file type. eg: .html, .htm, .md
    ...
    # CHECK 2.5: Ensure that the scraped url belongs to the source url domain
    if source_domain and tld not in source_domain:
        return False

    url_slug = path_chunks[-1] if path_chunks else ''

    # CHECK 3: check if bad domain
    ...
    # CHECK 4: return true if news slug title
