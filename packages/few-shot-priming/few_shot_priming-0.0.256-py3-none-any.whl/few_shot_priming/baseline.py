...
def run_finetuning_experiment_baseline(config=None, params=None, offline=True, validate=False, logger=None):
    splits = load_splits()
    use_cuda = torch.cuda.is_available()

    if use_cuda:
        log_message(logger, "-.-.-.- using cuda -.-.-.-.-", level=logging.WARNING)
    else:
        log_message(logger, "-.-..-. using cpu -.-.-.-.-..", level=logging.WARNING)
    ...
    if not params:
        ...
        log_message(logger, "Reading params from config file", level=logging.WARNING)
