...
def scatter_plot(tddf, x_col, y_col, **kwargs):
    ...
    **kwargs :
    ...
    y_range = kwargs.get('x_range', None)
...
def plot_with_background_image(dataset_large, dataset_small, col_x, col_y, x_range, y_range, color_small='red',
    ...
    # load the small dataset in a pandas dataframe
    dataset_small = dataset_small.to_pandas()

    # If color_small is a column name in dataset_small, replace it with the column's values
    if color_small in dataset_small.columns:
        color_small = dataset_small[color_small]

    # compute an image of the large dataset
    image = scatter_plot(dataset_large, col_x, col_y, x_range=x_range, y_range=y_range, noplot=True, color=color_large)
    ...
    # Create the hovertext if tooltip columns are provided
    ...
    # Determine the center and ranges for x and y
    center_x, center_y = dataset_small[col_x].mean(), dataset_small[col_y].mean()
    range_x, range_y = dataset_small[col_x].max() - dataset_small[col_x].min(), dataset_small[col_y].max() - \
                       dataset_small[col_y].min()

    # Set the x and y axis range based on the image's aspect ratio
    if width > height:
        aspect_ratio = width / height
        fig.update_xaxes(range=[center_x - range_x / 2 * aspect_ratio, center_x + range_x / 2 * aspect_ratio],
                         showgrid=False, zeroline=False, showline=False)
        fig.update_yaxes(range=[center_y - range_y / 2, center_y + range_y / 2], showgrid=False, zeroline=False,
                         showline=False)
    else:
        aspect_ratio = height / width
        fig.update_xaxes(range=[center_x - range_x / 2, center_x + range_x / 2], showgrid=False, zeroline=False,
                         showline=False)
        fig.update_yaxes(range=[center_y - range_y / 2 * aspect_ratio, center_y + range_y / 2 * aspect_ratio],
                         showgrid=False, zeroline=False, showline=False)
    ...
    fig.update_layout(
        ...
        width=600,
        height=600,
        title=title,  # Adding plot title
        xaxis_title=x_label,  # Adding x-axis label
