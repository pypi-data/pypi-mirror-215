    ...
    def execute_all(self, xt: np.ndarray) -> (np.ndarray, np.ndarray):
        ...
        for channel in range(channels):
            for frame in range(frames):
                ...
                if has_channels:
                    self._ft.execute(x[start:stop, channel], tmp, energy_t[channel, frame])
                    yf[:, channel, frame] = tmp
                else:
                    self._ft.execute(x[start:stop], tmp, energy_t[frame])
                    yf[:, frame] = tmp
            self.reset()

        return yf, energy_t

    def execute(self, xt: np.ndarray) -> (np.ndarray, np.single):
        assert xt.ndim == 1
        assert xt.shape[0] == self.R

        yf = np.empty(self._bins, dtype=np.csingle)
        energy_t = 0
        self._ft.execute(xt, yf, energy_t)
        return yf, energy_t
