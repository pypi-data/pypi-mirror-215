...
class Connection(object):
  ...
  def __init__(self, host='127.0.0.1', port=4367, flags=None,
    ...
    """
    if userid != '':
      _deprecated('userid', 'auth')

    if credentials != '':
      _deprecated('credentials', 'auth')
  ...
  def get_frames(self,names=None, namespace=None, frame_type=None):
    ...
    frame_type : str
      Selects the frame type returned: Edge, Vertex, or Table.  Must be one of 'edge', 'vertex',
      'table', or None. If None is selected it will return all types.
    ...
    if lc_frame_type == 'table':
      ...
      raise ValueError('frame_type must be one of edge, vertex, table or None, ' +
                       f'given: {format}')

    frames = []
    for data in response.container:
      schema, vertex_key, graph_type = self._translate_schema_from_server(data.schema)
      if graph_type == sch_proto.FrameTypeEnum.Value('TABLE'):
        frames.append(TableFrame(self, data.name, schema, data.container_id, data.commit_id))
      elif graph_type == sch_proto.FrameTypeEnum.Value('EDGE'):
      ...
      elif graph_type == sch_proto.FrameTypeEnum.Value('VERTEX'):
        frames.append(VertexFrame(self, data.name, schema, vertex_key, data.container_id, data.commit_id))
    return frames
  ...
  def get_frame(self, name):
    ...
    if len(frames) == 0:
      error_msg = ("Frame not found: {0}. It either does not "
  ...
  def get_vertex_frames(self, names = None, namespace = None):
    ...
    frames = []
    for data in response.container:
      schema, key, _ = self._translate_schema_from_server(data.schema)
  ...
  def create_vertex_frame(self, name, schema, key, frame_labels = None,
    ...
    for col in request.schema.property:
      if col.name == key:
        col.role = sch_proto.RoleEnum.Value('VERTEX_KEY')
    ...
    data = response.container[0]
    schema_returned, key_returned, _ = self._translate_schema_from_server(data.schema)
  ...
  def create_edge_frame(self, name, schema, source, target, source_key,
    ...
    for col in request.schema.property:
      if col.name == source_key:
        col.role = sch_proto.RoleEnum.Value('EDGE_SOURCE_KEY')
      elif col.name == target_key:
        col.role = sch_proto.RoleEnum.Value('EDGE_TARGET_KEY')
  ...
  def _translate_schema_from_server(self, schema_response):
    ...
    src_key_val = sch_proto.RoleEnum.Value('EDGE_SOURCE_KEY')
    trg_key_val = sch_proto.RoleEnum.Value('EDGE_TARGET_KEY')
    vertex_key = None
    graph_type = sch_proto.FrameTypeEnum.Value('TABLE')
    for prop in schema_response.property:
      ...
      if vertex_key_val == prop.role:
        ...
        graph_type = sch_proto.FrameTypeEnum.Value('VERTEX')
      elif src_key_val == prop.role or trg_key_val == prop.role:
        graph_type = sch_proto.FrameTypeEnum.Value('EDGE')

    return (schema, vertex_key, graph_type)
  ...
  def get_schema_from_data(self, data_source, header_mode = HeaderMode.NONE,
    ...
    if frame_to_file_column_mapping is not None:
      _deprecated('frame_to_file_column_mapping', 'frame_to_input_column_mapping')
  ...
  def create_edge_frame_from_data(self, data_source, name,
    ...
    def _check_and_create_endpoint_vertex_frame(vertex_frame_name, vertex_key, schema):
      existing_vertex_frames = [f.name for f in self.get_frames(frame_type = 'vertex')]
      key_type = None
