...
class _ContainerMap:
  ...
  def get_frame(self, row_id):
    # The row_id is a 64 bit unsigned int where the 16 low bits are the
    # container ID and the 48 high bits are the row position.
    container_id = row_id & 0xFFFF
...
class RowID:
  ...
  def frame(self):
    """
    Returns the frame the RowID points into.
  ...
  def position(self):
    """
    Returns the row position the RowID points to.
    ...
    # The row_id is a 64 bit unsigned int where the 16 low bits are the
    # container ID and the 48 high bits are the row position.
    return self._row_id >> 16

  @property
  def _validation_id(self):
    """
    Returns the validation ID of the row id.
  ...
  def __eq__(self, obj):
    return self._row_id == obj._row_id and self._validationid == obj._validationid
...
def _deprecated(name, replacement, stacklevel = 0):
  ...
  warnings.warn(f'{name} is deprecated and will be removed in future versions. Use {replacement} instead.', DeprecationWarning, stacklevel = stacklevel + 3)
...
def _validated_schema(obj):
  ...
  try:
    ...
    raise XgtTypeError(
        'A schema must be a non-empty list of (property, type) pairs.')
...
def _assert_isstring(value):
  if not isinstance(value, str):
    msg = f"{str(value)} is not a string"
    raise TypeError(msg)
...
def _convert_row_id_list_pandas(container_map, value, depth = 0):
  ...
  if depth > 1:
    for i in range(len(value)):
      v[i] = _convert_row_id_list_pandas(container_map, value[i], depth - 1) \
             if value[i] is not None else None
  else:
    for i in range(len(value)):
      v[i] = RowID(container_map, value[i]) if value[i] is not None else None
...
def _convert_ip_address_list_pandas(value, depth = 1):
  ...
  if depth > 1:
    for i in range(len(value)):
      v[i] = _convert_ip_address_list_pandas(value[i], depth - 1) \
             if value[i] is not None else None
  else:
    for i in range(len(value)):
      v[i] = ipaddress.ip_address(value[i]) if value[i] is not None else None
...
def _get_data_pandas_from_table(arrow_table, schema, conn):
  ...
  for i, (name, col) in enumerate(zip(arrow_table.column_names, arrow_table)):
    if i < len(schema) and schema[i][1] == IPADDRESS:
      ...
      pandas_dict[name] = \
          pd.Series([None if x.as_py() is None else RowID(container_map, x.as_py())
                     for x in col], dtype = 'object')
      ...
      pandas_dict[name] = \
          pd.Series([None if x.as_py() is None else
                     _convert_row_id_list_pandas(container_map, x.as_py(), depth)
                     for x in col], dtype = 'object')
    else:
