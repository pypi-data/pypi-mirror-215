...
class Job(object):
  ...
  def schema(self):
    """
    list of lists: Gets the property names and types of the stored results.
    """
    if not self._is_status_final():
      self._get_job_data()
  ...
  def __str__(self):
    txt = f'id:{self.id}, user:{self.user}, description:{self.description}, start:{self.start_time}, stop:{self.end_time}, status:{self.status}'
    if len(self.error) > 0:
