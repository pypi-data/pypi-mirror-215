...
from .common import (_assert_isstring, _assert_noerrors,
                     ...
                     _validated_schema, _validated_columns,
...
def _get_pandas_array_types():
  ...
  return (pd.arrays.PandasArray, pd.arrays.BooleanArray, pd.arrays.IntegerArray,
...
def _infer_xgt_type_from_pandas_column(table, col):
  ...
  def find_array_element_type(array, depth):
    ...
    raise xgt.XgtTypeError('Type not found for array element: ' + type(elem))
  ...
  if xgt_infer_type == 'pandasarray' and xgt_infer_subtype == 'object':
    for i in range(len(table[col])):
      if (table[col][i] is not None and
          isinstance(table[col][i], (IPv4Address, IPv6Address))):
...
def _infer_xgt_schema_from_python_data(data):
  ...
  def get_scalar_type(elem):
    ...
    raise XgtTypeError('Unsupported Python type for xGT data: ' +
                           str(type(elem)))
  ...
  def get_schema_for_row(row):
    ...
    for i, col in enumerate(row):
      if isinstance(col, list):
        schema.append(('f' + str(i),) + get_list_type(col, 0))
      else:
        schema.append(('f' + str(i),) + get_scalar_type(col))
  ...
  for i in range(1, len(data)):
    ...
    if schema != base_schema:
      raise XgtTypeError('Mixed type columns not supported for xGT ' +
                         'schema inference, expected schema ' +
                         str(base_schema) + ', row ' + str(i) +
                         ' schema ' + str(schema))
...
class TableFrame(object):
  ...
  def load(self, paths,
           ...
           frame_to_file_column_mapping = None, input_filter = None):
    """
    ...
    frame_to_file_column_mapping : dictionary
      ...
      .. deprecated:: 1.13.0
        Use frame_to_input_column_mapping instead.
    input_filter : str
  ...
  def _load(self, paths, header_mode = HeaderMode.NONE, record_history = True,
    ...
    if frame_to_file_column_mapping is not None:
      _deprecated('frame_to_file_column_mapping',
                  'frame_to_input_column_mapping', stacklevel = 1)
    ...
    if input_filter is not None and not isinstance(input_filter, str):
    ...
    if len(client_paths) > 0:
      job = self._local_ingest(client_paths, header_mode, row_labels,
                               ...
                               input_filter = input_filter, **kwargs)
  ...
  def insert(self, data, row_labels = None,
             ...
             suppress_errors = False, input_filter = None):
    """
    ...
    table, invalid_rows = self._build_arrow_table(
      ...
      frame_to_input_column_mapping, input_filter is not None)
  ...
  def _build_arrow_table(self, data, row_labels,
    ...
    if not (is_pandas or is_iterable or is_pyarrow_table):
      raise TypeError('a list of lists, a pandas DataFrame, or a pyarrow Table is expected')
  ...
  def _build_flight_path(self, row_labels, row_label_columns, suppress_errors,
    ...
    if input_filter is not None:
      input_filter_value = f'.input_filter="{input_filter}"'
      path += (input_filter_value,)
  ...
  def get_data(self, offset = 0, length = None, columns = None,
    ...
    columns = _validated_columns(columns, self._schema)
    ...
    if lc_format == 'python':
      return self._get_data(offset, length, columns, include_row_labels)
  ...
  def _get_data(self, offset = 0, length = None, columns = None,
  ...
  def _create_csv_packet (self, frame_name, header_mode, row_labels,
                          ...
                          on_duplicate_keys, input_filter, **kwargs):
  ...
  def _save_to_csv(self, path, offset = 0, length = None, headers = False,
                   include_row_labels = False, row_label_column_header = None,
                   preserve_order = False, delimiter: str = ',', **kwargs):
    ...
    responses = self._get_data_csv(
        ...
        row_label_column_header = row_label_column_header,
        preserve_order = preserve_order, delimiter = delimiter, **kwargs)
  ...
  def row_label_universe(self):
    """list of str: Gets the universe of row security labels that can be
...
class VertexFrame(TableFrame):
  ...
  @property
  def key_column(self):
    """str: Gets the column position of the frame's key."""
    return self._keycol
  ...
  def insert(self, data, row_labels = None,
             ...
             on_duplicate_keys = 'error', input_filter = None):
    """
    Inserts data rows. The properties of the new data must match the schema in
    both order and type.
    ...
    path = self._build_flight_path(
      row_labels, row_label_columns, suppress_errors,
      frame_to_input_column_mapping = frame_to_input_column_mapping,
      on_duplicate_keys = on_duplicate_keys, input_filter = input_filter)
    return self._write_table_to_flight(table, path, invalid_rows)
...
class EdgeFrame(TableFrame):
  ...
  def __init__(self, conn, name, schema, source, target, source_key,
    ...
    for i, entry in enumerate(schema):
      if entry[0] == source_key:
        self._source_keycol = i
      if entry[0] == target_key:
        self._target_keycol = i
  ...
  def source_key_column(self):
    """str: Gets the column position of the frame's source key."""
    return self._source_keycol

  @property
  def target_key_column(self):
    """str: Gets the column position of the frame's target key."""
    return self._target_keycol
  ...
  def insert(self, data, row_labels = None,
    ...
    table, invalid_rows = self._build_arrow_table(
      ...
      frame_to_input_column_mapping, input_filter is not None)
    path = self._build_flight_path(
      row_labels, row_label_columns, suppress_errors, source_vertex_row_labels,
      target_vertex_row_labels,
      frame_to_input_column_mapping = frame_to_input_column_mapping,
      input_filter = input_filter)
    return self._write_table_to_flight(table, path, invalid_rows)
