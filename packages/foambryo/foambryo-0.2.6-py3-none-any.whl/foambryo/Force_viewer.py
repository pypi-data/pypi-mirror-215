...
def plot_force_inference(Mesh,dict_tensions = None, dict_pressure = None,alpha = 0.05, scalar_quantities = False,scattered = False,scatter_coeff=0.2):
    ...
    ps_mesh = view_dict_values_on_mesh(Mesh,dict_tensions,alpha = alpha,ps_mesh = None, scattered=scattered, clean_before = False, clean_after=False,show=False,name_values = "Surface Tensions")
    ps_mesh = view_pressures_on_mesh(Mesh,dict_pressure,ps_mesh = ps_mesh, alpha = alpha, clean_before=False, clean_after=False, show = False,scattered=scattered)
    ...
    if scalar_quantities : 
        ps_mesh = view_dict_values_on_mesh(Mesh,dict_sphere_fit_residual,alpha = alpha,ps_mesh = ps_mesh,scattered=scattered, clean_before = False, clean_after=False,show=False,name_values = "Sphere fit residual")
        view_area_derivatives(Mesh, alpha = alpha,ps_mesh = ps_mesh, clean_before = False, clean_after = False,show=False,scattered=scattered)
        view_volume_derivatives(Mesh, alpha = alpha,ps_mesh = ps_mesh, clean_before = False, clean_after = False,show=False,scattered=scattered)
        view_mean_curvature_cotan(Mesh, alpha = alpha,ps_mesh = ps_mesh, clean_before = False, clean_after = False,show=False,scattered=scattered)
        view_mean_curvature_robust(Mesh, alpha = alpha,ps_mesh = ps_mesh, clean_before = False, clean_after = False,show=False,scattered=scattered)
        view_gaussian_curvature(Mesh, alpha = alpha,ps_mesh = ps_mesh, clean_before = False, clean_after = False,show=False,scattered=scattered)
        view_discrepancy_of_principal_curvatures(Mesh, alpha = alpha,ps_mesh = ps_mesh, clean_before = False, clean_after = False,show=False,scattered=scattered)

    ps.show()
    

def plot_tension_inference(Mesh,dict_tensions=None,alpha = 0.05, scalar_quantities = False,scattered = False,scatter_coeff=0.2):
    ...
    display_embryo_graph_forces(G,alpha = alpha, clean_before = False, clean_after = False,show=False,P0 = 0, Plot_pressures = False)
    if scalar_quantities : 
        ps_mesh = view_dict_values_on_mesh(Mesh,dict_sphere_fit_residual,alpha = alpha,ps_mesh = ps_mesh,scattered=scattered, clean_before = False, clean_after=False,show=False,name_values = "Sphere fit residual")
        view_area_derivatives(Mesh, alpha = alpha,ps_mesh = ps_mesh, clean_before = False, clean_after = False,show=False,scattered=scattered)
        view_volume_derivatives(Mesh, alpha = alpha,ps_mesh = ps_mesh, clean_before = False, clean_after = False,show=False,scattered=scattered)
...
def view_pressures_on_mesh(Mesh,dict_pressures,alpha = 0.05,ps_mesh = None, clean_before = True, clean_after=True,show=True, scattered = False):
    ...
    if scattered:
        v,f,cluster_idx = Mesh.v_scattered,Mesh.f_scattered,Mesh.cluster_idx_scattered#, Mesh.idx_scattered
...
def view_area_derivatives(Mesh,alpha = 0.05,ps_mesh = None, remove_trijunctions=True,clean_before = True, clean_after = True,show=True,scattered=False):
    DA = Mesh.compute_area_derivatives()
    Derivatives_verts = np.zeros(Mesh.v.shape)
    for key in DA.keys() : 
        Derivatives_verts +=np.abs(DA[key])

    vmin, vmax = np.quantile(Derivatives_verts,alpha),np.quantile(Derivatives_verts,1-alpha)
    print("Extremas of area derivatives plotted: ",vmin,vmax)
    Derivatives_verts = Derivatives_verts.clip(vmin,vmax)
    
    ps_mesh = view_vertex_values_on_embryo(Mesh,Derivatives_verts,name_values="Area Derivatives",ps_mesh = ps_mesh,remove_trijunctions=remove_trijunctions,clean_before = clean_before, clean_after = clean_after,show=show, scattered = scattered)
    
    return(ps_mesh)

def view_volume_derivatives(Mesh,alpha = 0.05,ps_mesh = None, remove_trijunctions=True,clean_before = True, clean_after = True,show=True,scattered=False):
...
def view_mean_curvature_cotan(Mesh,alpha = 0.05,ps_mesh = None, remove_trijunctions=True,clean_before = True, clean_after = True,show=True,scattered = False):
    ...
    ps_mesh = view_vertex_values_on_embryo(Mesh,H,ps_mesh = ps_mesh,name_values="Mean Curvature Cotan",remove_trijunctions=remove_trijunctions,clean_before = clean_before, clean_after = clean_after,show=show, scattered = scattered)
        
    return(ps_mesh)

def view_gaussian_curvature(Mesh,alpha = 0.05,ps_mesh = None, remove_trijunctions=True,clean_before = True, clean_after = True,show=True,scattered = False):
    ...
    ps_mesh = view_vertex_values_on_embryo(Mesh,H,ps_mesh = ps_mesh,name_values="Gaussian Curvature",remove_trijunctions=remove_trijunctions,clean_before = clean_before, clean_after = clean_after,show=show, scattered = scattered)
...
def view_discrepancy_of_principal_curvatures(Mesh,alpha = 0.05,ps_mesh = None, remove_trijunctions=True,clean_before = True, clean_after = True,show=True,scattered = False):
    ...
    ps_mesh = view_vertex_values_on_embryo(Mesh,Kdiff,ps_mesh = ps_mesh,name_values="Principal curvature discrepancy",remove_trijunctions=remove_trijunctions,clean_before = clean_before, clean_after = clean_after,show=show, scattered = scattered)
...
def display_embryo_graph(Mesh,clean_before = True, clean_after = True,show=True):
    ...
    ps.register_curve_network("Edges",Centroids_for_plotting,Edges_for_plotting,color = [0.5,0.5,0.5],radius=0.02)
...
def display_embryo_graph_forces(G,alpha = 0.05, clean_before = True, clean_after = True,show=True,P0=0, Plot_pressures = True):
    ...
    ps_net = ps.register_curve_network("Graph Edges",vertices,edges,radius=0.007, color = np.array([92,85,141])/255)#,color = "000000")
    ps_net.add_color_quantity("Surface tensions", colors_tensions, defined_on='edges',enabled = True)

    
    Centroids_for_plotting = np.array([a[1] for a in G.nodes.data('centroid')])[np.array(G.nodes.data('volume'))[:,1]>0]#[1:]
    ...
    if Plot_pressures : 
        Pressure_for_plotting = np.array([a[1] for a in G.nodes.data('pressure')])[np.array(G.nodes.data('volume'))[:,1]>0]#[1:]
...
def plot_valid_junctions(Mesh,dict_tensions=None):
    ...
    if np.amax(list(dict_validity.values()))==0:
        plot_trijunctions_topo_change_viewer(Mesh,Dict_trijunctional_values=dict_validity,color="uniform",value_color = (0,1,0),clean_before=False)
    else:
...
def plot_residual_junctions(Mesh,dict_tensions=None,alpha = 0.05):
    ...
    print("Extremas of the residuals plotted : ",np.amin(list(dict_residuals.values())),np.amax(list(dict_residuals.values())))
