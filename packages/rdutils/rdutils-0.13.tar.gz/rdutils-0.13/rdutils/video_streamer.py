...
class VideoStreamer:
    """Object to manage threaded and unthreaded streaming of video from RTSP or file sources
    """
    def __init__(self, cam_file_path, frame_size=(None, None), identifier=1, threaded=True):
        """Opens a video file
        :param cam_file_path: string video file location/RTSP Stream address (with authentication)
        ...
        :param threaded: (OPTIONAL) bool determining if the video should stream in a separate thread
        """
    ...
    def _read_frame(self):
        """Gets the next frame and will resize the frame accordingly
        """
    ...
    def _update(self):
        """Updates the self.current_frame with the latest frame from the streamer object and the
        self.grabbed bool will close the thread if the stream fails.
        """
        while True:
            if self.stopped:
                return
    ...
    def stop(self):
        """Sets the self.stopped bool to True but doesn't close the thread,
        stopping the reading of frames
        """
        self.stopped = True
...
if __name__ == '__main__':
    ...
    while cap.open():
        ...
        if cv2.waitKey(1) == ord('c') or not cap.grabbed:
            exit(0)
        # SLEEP TO ENSURE THREADING IS WORKING (COUNTER SHOULD GO UP OUTSIDE OF READ CALLS)
        time.sleep(1)
