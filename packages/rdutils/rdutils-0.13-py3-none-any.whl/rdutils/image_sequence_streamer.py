...
""" Loads an Image sequence and returns each frame sequentially when called. Provides options for
looping the stream or repeating the last frame"""
...
class ImageSequenceStreamer:
    def __init__(self, seq_path, start_frame=0, identifier=0, frame_size=(None, None),
                 loop_last=True, repeat=False, write_to_file=False):
        """Loads an Image sequence and returns each frame sequentially when called. Provides
        options for looping the
        ...
        try:
            ...
            if write_to_file:
                ...
                self.out = cv2.VideoWriter(f'{seq_path}video.mp4', self.fourcc, 25, (self.width, self.height))

                if self.loop_last or self.repeat:
                    print('Sequence has been set to write to file and is also set to loop last frame or repeat sequence, this will lead to large files unless terminated')
    ...
    def release(self):
        """releases the video stream writer
        """
        self.out.release()
