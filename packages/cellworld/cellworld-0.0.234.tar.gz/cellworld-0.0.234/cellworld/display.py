...
class Display:
    ...
    def add_trajectories(self, trajectories: Trajectories, colors={}, alphas={}, zorder=4, auto_alpha=False, distance_equalization: int = 0) -> dict:
        agents_locations = {}
        split = trajectories.split_by_agent()
        for index, agent in enumerate(split):
            agent_trajectory = split[agent]
            ...
            if distance_equalization > 0:
                agents_locations[agent] = StreamLine(trajectory=agent_trajectory, streamline_len=distance_equalization)
            else:
                agents_locations[agent] = agent_trajectory.get_locations()[agent]

            if auto_alpha:
                dist = agents_locations[agent].distance_from_start()
                if not dist or dist[-1] == 0:
                    alpha = 1
                else:
                    alpha = [d/dist[-1]/2 for d in dist]

            for i in range(len(agents_locations[agent])-1):
                ...
                if type(color) is numpy.ndarray:
                    ...
                    lcolor = color
                self.ax.plot([agents_locations[agent][i].x, agents_locations[agent][i+1].x], [agents_locations[agent][i].y, agents_locations[agent][i+1].y], color=lcolor, alpha=lalpha, linewidth=3, zorder=zorder)
    ...
    def add_icon(self, location: Location, icon_name: str="", icon_file_path: str="", rotation: float = 0, size: float = .05, zorder: int = 10, alpha:float = 1):
        ...
        self.ax.imshow(rotated_img, extent=Agent_markers.icon_box(location, .05, rotation), alpha=alpha, zorder=zorder)
...
def EpisodeRepresentation(episode: Episode, world: World, frames: list = None, time_stamps: list = None, colors={"prey": "blue", "predator": "red"}, cols=0, figsize=(10,10), video_path:str = "", video_frames:list=None, icons:dict={"prey":"mouse", "predator":"robot"}, **kwargs):
    ...
    for i, frame in enumerate(frames):
        ...
        displays[-1].add_trajectories(trajectories, colors=colors, auto_alpha=True)
        if not video_frames:
            for agent in split:
                step = split[agent].get_step_by_frame(frame)
                if "." in icons[agent]:
                    displays[-1].add_icon(step.location, icon_file_path=icons[agent], rotation=step.rotation)
                else:
                    displays[-1].add_icon(step.location, icon_name=icons[agent], rotation=step.rotation)
        ps = frame
