...
class Display:
    ...
    def add_trajectories(self,
                         ...
                         distance_equalization: int = 0) -> dict:
        agents_locations = {}
        ...
        for index, agent in enumerate(split):
            ...
            segments = []
            previous = agents_locations[agent][0]

            for location in agents_locations[agent][1:]:
                segments.append([[previous.x, previous.y], [location.x, location.y]])
                previous = location
            ...
            colorfade = mcolors.to_rgb(color) + (0.0,)
            mycolor = mcolors.LinearSegmentedColormap.from_list('my', [colorfade, color])
            lc = LineCollection(segments, array=step_alphas, cmap=mycolor, lw=3, zorder=zorder)
    ...
    def agent(self, step: Step = None, agent_name: str = None, location: Location = None, rotation: float = None, color=None, size: float = 40.0,  zorder: int = -6, marker: Path=None):
        if step:
