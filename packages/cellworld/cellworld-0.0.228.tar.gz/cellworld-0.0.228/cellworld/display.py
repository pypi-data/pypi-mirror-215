...
class Display:
    ...
    def add_trajectories(self, trajectories: Trajectories, colors={}, alphas={}, start_frame=0, end_frame=None, start_time=0, end_time=None, zorder=4, auto_alpha=False) -> dict:
        agents = trajectories.get_agent_names()
        agents_locations = {}
        for index, agent in enumerate(agents):
            agents_locations[agent] = Location_list()
            ...
            for step in agent_trajectory:
                ...
                agents_locations[agent].append(step.location)
            ...
            if agent in alphas:
                alpha = alphas[agent]
            if auto_alpha:
                dist = agents_locations[agent].distance()
...
def EpisodeRepresentation(episode: Episode, world: World, frames: list = None, time_stamps: list = None, colors={"prey": "blue", "predator": "red"}, cols=0, figsize=(10,10), video_path:str = "", video_frames:list=None, icons:dict={"prey":"mouse", "predator":"robot"}, **kwargs):
    ...
    for i, frame in enumerate(frames):
        ...
        agent_locations = displays[-1].add_trajectories(episode.trajectories, colors=colors, start_frame=ps, end_frame=frame, auto_alpha=True)
        if not video_frames:
            for agent in split:
