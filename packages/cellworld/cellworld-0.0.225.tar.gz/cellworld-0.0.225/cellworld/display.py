...
class Display:
    ...
    def agent_icon (self, icon_name: str, location: Location, rotation: float, size=.05, zorder=10, alpha=1):
...
def EpisodeRepresentation(episode: Episode, world: World, frames: list=None, time_stamps: list=None, colors={"prey": "blue", "predator": "red"}, cols=0, figsize=(10,10), video_path:str="", video_frames:list=None, **kwargs):
    if video_path:
        ...
        for ts in time_stamps:
            for i, step in enumerate(episode.trajectories[pf:]):
                if step.time_stamp > ts:
                    selection_frames.append(step.frame)
                    break
                pf = i
            else:
                selection_frames.append(step.frame)
    ...
    for i, selection in enumerate(selections):
        ...
        if video_frames:
            ...
            displays[-1].set_background(video_frames[selection_frames[i]], 1)
        else:
        ...
        if frames:
            displays[-1].add_trajectories(episode.trajectories, colors=colors, start_frame=ps, end_frame=selection, auto_alpha=True)
        else:
            displays[-1].add_trajectories(episode.trajectories, colors=colors, start_time=ps, end_time=selection, auto_alpha=True)
        ps = selection
