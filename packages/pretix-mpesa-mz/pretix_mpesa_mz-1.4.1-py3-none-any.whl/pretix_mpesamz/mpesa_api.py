...
class APIRequest:
    ...
    def create_default_headers(self):
        self.context.add_header('Authorization', 'Bearer {}'.format(
            self.create_bearer_token().decode('utf-8')))
        self.context.add_header('Content-Type', 'application/json')
        self.context.add_header('Host', self.context.address)

    def __get(self):
        r = requests.get(self.context.get_url(
        ), params=self.context.get_parameters(), headers=self.context.get_headers())
        print(r)
    ...
    def __post(self):
        ...
        ), headers=self.context.get_headers(), json=self.context.get_parameters())
        print(r)
        return APIResponse(r.status_code, json.loads(r.headers.__str__().replace("'", '"')), json.loads(r.text))
...
def execute_mpesa_c2b(config, payload):
    ...
    api_context.add_parameter('input_CustomerMSISDN', payload['msisdn'])
    api_context.add_parameter('input_Amount', payload['amount'])
    api_context.add_parameter(
        'input_ThirdPartyReference', payload['third_party_reference'])
    api_context.add_parameter(
        'input_ServiceProviderCode', payload['service_provider_code'])
...
def execute_mpesa_reversal(config, payload):
    api_context = APIContext()
    api_context.api_key = config['api_key']
    api_context.public_key = config['public_key']
    api_context.ssl = True
    ...
    api_context.port = 18354
    api_context.path = '/ipg/v1x/reversal/'

    api_context.add_header('Origin', '*')

    api_context.add_parameter('input_TransactionID', payload['transaction'])
    api_context.add_parameter('input_SecurityCredential', 'Mpesa2019')
    api_context.add_parameter('input_InitiatorIdentifier', 'MPesa2018')
    api_context.add_parameter(
        'input_ThirdPartyReference', payload['third_party_reference'])
    api_context.add_parameter(
        'input_ServiceProviderCode', payload['service_provider_code'])
    api_context.add_parameter('input_ReversalAmount', payload['amount'])
