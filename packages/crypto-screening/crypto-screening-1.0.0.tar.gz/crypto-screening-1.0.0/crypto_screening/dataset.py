...
from typing import (
    ...
    Any, Callable, Dict, LiteralString
)
...
Extension = LiteralString[CSV_EXTENSION, JSON_EXTENSION]

def validate_file_formatting(
        path: Union[str, Path],
        formatting: Optional[Extension] = None
) -> str:
    """
    ...
    if formatting not in EXTENSIONS:
        raise ValueError(
...
def save_dataset(
    ...
    extension = validate_file_formatting(path=path, formatting=extension)
