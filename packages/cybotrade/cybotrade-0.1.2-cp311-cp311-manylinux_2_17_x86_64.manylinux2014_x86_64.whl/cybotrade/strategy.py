...
from .models import Candle, Performance, RuntimeMode
...
class Strategy:
    ...
    async def on_candle_closed(self, strategy, candle: Candle, candles: List[Candle]):
        # No need to log if it's backtest
        if strategy.config.mode == RuntimeMode.Backtest:
            return

        logging.info(
            f"[{self.__class__.__name__}] Candle for {candle.symbol.base}/{candle.symbol.quote} beginning at {candle.start_time} has closed at {candle.close}. "
            + f"Maintaining a total of {len(candles)} candles"
        )
