...
class XLtestData(TestData):
    ...
    try:
        ...
        print(colored("No workbook file found",'red'))
    ...
    # For Automation sheet_name value will be passed as argument
    # If there is not argument for this method then User input will be required.
    ...
    def __arrange_data_by_variable_value_list(self):
        ...
        for curr_column in range(1, columns+1):
            # Traversing each Column cell of a Row.
            for curr_row in range(1,rows+1):
                # Getting a cell value (Converted into string).
                current_cell_value = str( \
                    self.sheet_file.cell(\
                        row= curr_row, column = curr_column \
                            ).value \
                        )
                """
                ...
                if curr_row == 1:
                    ...
                    return colored("Something went wrong! please re-start the application and try again.",'red')
