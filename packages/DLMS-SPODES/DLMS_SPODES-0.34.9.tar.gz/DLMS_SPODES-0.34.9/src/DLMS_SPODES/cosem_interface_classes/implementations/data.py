...
class ActiveFirmwareId(Data):
    ...
    def characteristics_init(self):
        self._cbs_attr_post_init.update({2: lambda: self.value.register_cb_preset(self.__handle_value)})

    def __handle_value(self, value: cdt.CommonDataTypes):
        """check new version for more than current collection_version"""
        version = AppVersion.from_str(value.contents.decode("utf-8"))
        collection_ver = self.collection.server_ver.get(self.logical_name.b, AppVersion(0, 0))
        if version > collection_ver:
            raise ValueError(F"got {version}, but used {collection_ver}. need update server configuration")
