...
from cmon.clients.mixin import PostMixin
from cmon.constants import (
...
from cmon.enums import DeploymentRoleType, PodRoleType, PollingType, ProtocolType
...
from cmon.orchestrate.deployments.install_requirements_helper import (
...
from cmon.parsers import set_deployment_parser, set_gateway_parser
from cmon.parsers.helper import _update_gateway_args
from cmon.serve.networking import GrpcConnectionPool
from cmon.serve.networking.utils import host_is_local, in_docker
...
def _call_add_voters(leader, voters, replica_ids, name, event_signal=None):
    ...
    logger = CmonLogger(context=f'add_voter-{name}', name=f'add_voter-{name}')
...
class Deployment(JAMLCompatible, PostMixin, BaseOrchestrator, metaclass=DeploymentType):
    ...
    class _ReplicaSet:
        def __init__(
            ...
            self.logger = CmonLogger(context=self.name, name=self.name, **logger_kwargs)
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.needs = (
            needs or set()
        )  #: used in the :class:`cmon.flow.Flow` to build the graph
        ...
        self.logger = CmonLogger(self.__class__.__name__, **vars(self.args))
    ...
    def _docker_compose_address(self):
        from cmon.orchestrate.deployments.config.docker_compose import port
    ...
    def _to_docker_compose_config(self, deployments_addresses=None):
        from cmon.orchestrate.deployments.config.docker_compose import (
            DockerComposeConfig,
        )
    ...
    def to_docker_compose_yaml(
        ...
        """
        Converts a Cmon Deployment into a Docker compose YAML file
    ...
    def _to_kubernetes_yaml(
        ...
        from cmon.orchestrate.deployments.config.k8s import K8sDeploymentConfig
    ...
    def to_kubernetes_yaml(
        ...
        """
        Convert a Cmon Deployment into a set of YAML deployments to deploy in Kubernetes.
