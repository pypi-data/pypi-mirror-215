...
from cmon.orchestrate.deployments.config.k8slib import kubernetes_tools
...
def get_template_yamls(
    ...
    :param cmon_deployment_name: Name of the Cmon Deployment this deployment belongs to
    ...
    :param volumes: If volumes are passed to Executors, Cmon will create a StatefulSet instead of Deployment and include the first volume in the volume mounts
    ...
    template_params = {
        ...
        'cmon_deployment_name': cmon_deployment_name,
    ...
    if volumes:
        ...
        template_params['accessModes'] = json.loads(
            os.environ.get('CMON_K8S_ACCESS_MODES', '["ReadWriteOnce"]')
        )
        template_params['storageClassName'] = os.environ.get(
            'CMON_K8S_STORAGE_CLASS_NAME', 'standard'
        )
        template_params['storageCapacity'] = os.environ.get(
            'CMON_K8S_STORAGE_CAPACITY', '10G'
        )
    ...
    if 'CMON_LOG_LEVEL' in os.environ:
        env = env or {}
        env['CMON_LOG_LEVEL'] = os.environ['CMON_LOG_LEVEL']
