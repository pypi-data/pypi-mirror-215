...
from cmon._docarray import DocumentArray, docarray_v2
from cmon.excepts import BadRequestType
from cmon.helper import random_identity, typename
from cmon.proto import cmon_pb2
from cmon.types.request import Request

if TYPE_CHECKING:
    from cmon._docarray import Document, DocumentArray, docarray_v2

RequestSourceType = TypeVar(
    'RequestSourceType', cmon_pb2.DataRequestProto, str, Dict, bytes
)
...
class DataRequest(Request):
    ...
    When calling :meth:`~cmon.clients.mixin.PostMixin.post` on any Cmon client,
    ...
    class _DataContent:
        def __init__(
            self,
            content: 'cmon_pb2.DataRequestProto.DataContentProto',
    ...
    """
    :class:`DataRequest` is one of the **primitive data type** in Cmon.

    It offers a Pythonic interface to allow users access and manipulate
    :class:`cmon.cmon_pb2.DataRequestProto` object without working with Protobuf itself.

    A container for serialized :class:`cmon_pb2.DataRequestProto` that only triggers deserialization
    ...
    :class:`cmon_pb2.DataRequestProto` object.
    ...
    def __init__(
        ...
        try:
            if isinstance(request, cmon_pb2.DataRequestProto):
                self._pb_body = request
            elif isinstance(request, dict):
                self._pb_body = cmon_pb2.DataRequestProto()
    ...
    def is_decompressed(self) -> bool:
        """
        Checks if the underlying proto object was already deserialized into a :class:`cmon.proto.cmon_pb2.DataRequestProto` or
        :class:`cmon.proto.cmon_pb2.DataRequestProtoWoData`. This does not necessarily mean that the data (docs) inside the request is also decompressed.
           :return: True if the proto was deserialized before
        """
        return type(self._pb_body) in [
            cmon_pb2.DataRequestProto,
            cmon_pb2.DataRequestProtoWoData,
        ]

    @property
    def is_decompressed_with_data(self) -> bool:
        """
        Checks if the underlying proto object was already deserialized into a :class:`cmon.proto.cmon_pb2.DataRequestProto`. In this case the full proto is decompressed, including the data (docs).
           :return: True if the proto was deserialized before, including the data (docs)
        """
        return type(self._pb_body) is cmon_pb2.DataRequestProto

    @property
    def is_decompressed_wo_data(self) -> bool:
        """
        Checks if the underlying proto object was already deserialized into a :class:`cmon.proto.cmon_pb2.DataRequestProtoWoData`. It means that the proto is loaded without the data ( docs ).

        :return: True if the proto was deserialized before into a DataRequest without docs
        """
        return type(self._pb_body) is cmon_pb2.DataRequestProtoWoData

    @property
    def proto_wo_data(
        self,
    ) -> Union['cmon_pb2.DataRequestProtoWoData', 'cmon_pb2.DataRequestProto']:
        """
        Transform the current buffer to a :class:`cmon_pb2.DataRequestProtoWoData` unless the full proto has already
    ...
    def proto(
        self,
    ) -> Union['cmon_pb2.DataRequestProto', 'cmon_pb2.DataRequestProtoWoData']:
        """
        Cast ``self`` to a :class:`cmon_pb2.DataRequestProto` or a :class:`cmon_pb2.DataRequestProto`. Laziness will be broken and serialization will be recomputed when calling.
    ...
    def proto_with_data(
        ...
        """
        Cast ``self`` to a :class:`cmon_pb2.DataRequestProto`. Laziness will be broken and serialization will be recomputed when calling.
        :meth:`SerializeToString`.
        :return: DataRequestProto protobuf instance
        """
    ...
    def _decompress_wo_data(self):
        ...
        self._pb_body = cmon_pb2.DataRequestProtoWoData()
    ...
    def to_dict(self) -> Dict:
        ...
        .. note::
            Array like object such as :class:`numpy.ndarray` (i.e. anything described as :class:`cmon_pb2.NdArrayProto`)
    ...
    def add_executor(self, executor_name: str):
        ...
        """
        route_proto = cmon_pb2.RouteProto()
    ...
    @classmethod
    def from_proto(cls, request: 'cmon_pb2.DataRequestProto'):
...
class Response(DataRequest):
    """
    Response is the :class:`~cmon.types.request.Request` object returned by the flow.

    At the moment it is an alias for :class:`~cmon.types.request.Request`,
    and therefore shares an identical representation.
    Currently, its sole purpose is to give a more consistent semantic on
    the client API: send a :class:`~cmon.types.request.data.DataRequest` and receive a :class:`~cmon.types.request.data.Response`.
    """
