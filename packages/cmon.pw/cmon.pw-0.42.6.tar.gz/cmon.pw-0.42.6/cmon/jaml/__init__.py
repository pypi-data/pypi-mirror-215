...
from cmon.jaml.helper import (
    CmonLoader,
    CmonResolver,
    get_cmon_loader_with_runtime,
    load_py_modules,
    parse_config_source,
)
...
class JAML:
    """A Cmon YAML parser supports loading and dumping and substituting variables.
    ...
    You need to use specific syntax to tell Cmon to evaluate an expression rather than treat it as a string,
    ...
    def load(
        ...
        """
        r = yaml.load(stream, Loader=get_cmon_loader_with_runtime(runtime_args))
    ...
    def registered_tags() -> List[str]:
        ...
        return list(
            v[1:]
            for v in set(CmonLoader.yaml_constructors.keys())
    ...
    def registered_classes() -> Dict:
        ...
        return {
            k[1:]: v
            for k, v in CmonLoader.yaml_constructors.items()
    ...
    def cls_from_tag(tag: str) -> Optional['JAMLCompatible']:
        ...
        if not tag.startswith('!'):
            tag = '!' + tag
        bound = CmonLoader.yaml_constructors.get(tag, None)
    ...
    def expand_dict(
        ...
        from cmon.helper import parse_arg
    ...
    def register(cls):
        ...
        try:
            yaml.add_constructor(tag, cls._from_yaml, CmonLoader)
        except AttributeError:
            ...
            yaml.add_constructor(tag, f_y, CmonLoader)
        return cls
...
class JAMLCompatible(metaclass=JAMLCompatibleType):
    ...
    def _to_yaml(cls, representer, data):
        ...
        """
        from cmon.jaml.parsers import get_parser

        config_dict = get_parser(cls, version=data._version).dump(data)
        config_dict_with_jtype = {
            'jtype': cls.__name__
        }  # specifies the type of Cmon object that is represented
