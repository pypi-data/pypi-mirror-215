"""Module wrapping the Client of Cmon."""
import argparse
from typing import TYPE_CHECKING, List, Optional, Union, overload

from cmon.helper import parse_client
...
if TYPE_CHECKING:  # pragma: no cover
    from cmon.clients.grpc import AsyncGRPCClient, GRPCClient
    from cmon.clients.http import AsyncHTTPClient, HTTPClient
    from cmon.clients.websocket import AsyncWebSocketClient, WebSocketClient
...
def Client(
    ...
    if protocol == ProtocolType.GRPC:
        if is_async:
            from cmon.clients.grpc import AsyncGRPCClient

            return AsyncGRPCClient(args, **kwargs)
        else:
            from cmon.clients.grpc import GRPCClient

            return GRPCClient(args, **kwargs)
    elif protocol == ProtocolType.WEBSOCKET:
        if is_async:
            from cmon.clients.websocket import AsyncWebSocketClient

            return AsyncWebSocketClient(args, **kwargs)
        else:
            from cmon.clients.websocket import WebSocketClient

            return WebSocketClient(args, **kwargs)
    elif protocol == ProtocolType.HTTP:
        if is_async:
            from cmon.clients.http import AsyncHTTPClient

            return AsyncHTTPClient(args, **kwargs)
        else:
            from cmon.clients.http import HTTPClient
