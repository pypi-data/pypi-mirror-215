...
from cmon.enums import DataInputType, WebsocketSubProtocols
...
def get_fastapi_app(
    ...
    """
    if not docarray_v2:
        from cmon.serve.runtimes.gateway.models import CmonEndpointRequestModel
    else:
    ...
    @app.get(
        path='/',
        summary='Get the health of Cmon service',
    )
    async def _health():
        """
        Get the health of this Cmon service.
        .. # noqa: DAR201
            ...
            async for request in manager.iter(websocket):
                if isinstance(request, dict):
                    if request == {}:
                        ...
                        if not docarray_v2:
                            req_generator_input = CmonEndpointRequestModel(**request).dict()
    ...
    from cmon.serve.runtimes.gateway.models import PROTO_TO_PYDANTIC_MODELS
