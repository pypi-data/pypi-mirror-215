...
func NewExecutorFSM(target string, LogLevel string, raftID string) *executorFSM {
    ...
    defer conn.Close()
    client := pb.NewCmonDiscoverEndpointsRPCClient(conn)
...
func (fsm *executorFSM) Apply(l *raft.Log) interface{} {
    ...
    defer conn.Close()
    client := pb.NewCmonSingleDataRequestRPCClient(conn)
...
func (fsm *executorFSM) Snapshot() (raft.FSMSnapshot, error) {
    ...
    defer conn.Close()
    client := pb.NewCmonExecutorSnapshotClient(conn)
...
func (fsm *executorFSM) Restore(r io.ReadCloser) error {
    ...
    defer conn.Close()
    client := pb.NewCmonExecutorRestoreClient(conn)
    ...
    go func(funcTicker *time.Ticker) {
        for {
            ...
            case t := <-funcTicker.C:
                ...
                if err == nil {
                    defer conn.Close()
                    client := pb.NewCmonExecutorRestoreProgressClient(conn)
...
func (fsm *executorFSM) XStatus(ctx context.Context, empty *empty.Empty) (*pb.CmonInfoProto, error) {
    ...
    defer conn.Close()
    client := pb.NewCmonInfoRPCClient(conn)
