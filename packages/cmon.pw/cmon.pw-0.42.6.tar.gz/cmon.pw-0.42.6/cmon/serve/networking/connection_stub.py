...
from cmon.serve.networking.utils import get_available_services, get_grpc_channel
...
class _ConnectionStubs:
    ...
    STUB_MAPPING = {
        'cmon.CmonDataRequestRPC': cmon_pb2_grpc.CmonDataRequestRPCStub,
        'cmon.CmonSingleDataRequestRPC': cmon_pb2_grpc.CmonSingleDataRequestRPCStub,
        'cmon.CmonDiscoverEndpointsRPC': cmon_pb2_grpc.CmonDiscoverEndpointsRPCStub,
        'cmon.CmonRPC': cmon_pb2_grpc.CmonRPCStub,
        'cmon.CmonInfoRPC': cmon_pb2_grpc.CmonInfoRPCStub,
    }
    ...
    async def _init_stubs(self):
        ...
        self.data_list_stub = stubs['cmon.CmonDataRequestRPC']
        self.single_data_stub = stubs['cmon.CmonSingleDataRequestRPC']
        self.stream_stub = stubs['cmon.CmonRPC']
        self.endpoints_discovery_stub = stubs['cmon.CmonDiscoverEndpointsRPC']
        self.info_rpc_stub = stubs['cmon.CmonInfoRPC']
    ...
    async def send_discover_endpoint(
        ...
        call_result = self.endpoints_discovery_stub.endpoint_discovery(
            cmon_pb2.google_dot_protobuf_dot_empty__pb2.Empty(),
            timeout=timeout,
        )
        metadata, response = (
    ...
    async def send_info_rpc(self, timeout: Optional[float] = None):
        """
        Use the CmonInfoRPC stub to send request to the _status endpoint exposed by the Runtime
        :param timeout: defines timeout for sending request
        :returns: CmonInfoProto
        """
        if not self._initialized:
            await self._init_stubs()
