from cmon.helper import GATEWAY_NAME
from cmon.parsers.helper import _SHOW_ALL_ARGS
from cmon.parsers.logging import mixin_suppress_root_logging_parser
from cmon.parsers.orchestrate.runtimes.container import mixin_container_runtime_parser
from cmon.parsers.orchestrate.runtimes.grpc_channel import (
    mixin_grpc_channel_options_parser,
)
from cmon.parsers.orchestrate.runtimes.head import mixin_head_parser
...
def set_pod_parser(parser=None, default_name=None):
    ...
    """
    if not parser:
        from cmon.parsers.base import set_base_parser

        parser = set_base_parser()

    from hubble.executor.parsers.pull import mixin_hub_pull_options_parser

    from cmon.parsers.orchestrate.base import mixin_scalable_deployment_parser
    from cmon.parsers.orchestrate.pod import mixin_pod_parser
    from cmon.parsers.orchestrate.runtimes.container import \
        mixin_container_runtime_parser
    from cmon.parsers.orchestrate.runtimes.remote import \
        mixin_remote_runtime_parser
    from cmon.parsers.orchestrate.runtimes.worker import \
...
def set_deployment_parser(parser=None):
    ...
    from cmon.parsers.orchestrate.deployment import \
...
def set_gateway_parser(parser=None):
    ...
    from cmon.parsers.orchestrate.base import mixin_base_deployment_parser
    from cmon.parsers.orchestrate.pod import mixin_pod_parser
    from cmon.parsers.orchestrate.runtimes.remote import (
    ...
    from cmon.enums import DeploymentRoleType
...
def set_gateway_runtime_args_parser(parser=None):
    ...
    from cmon.parsers.orchestrate.pod import mixin_pod_runtime_args_parser
    from cmon.parsers.orchestrate.runtimes.remote import (
        _add_host,
...
def set_client_cli_parser(parser=None):
    ...
    from cmon.parsers.client import (mixin_client_features_parser,
...
def set_help_parser(parser=None):
    """Set the parser for the cmon help lookup
    ...
    parser.add_argument(
        'query',
        type=str,
        help='Look up usage & mention of argument name in Cmon API. The name can be fuzzy',
    )
    return parser


def get_main_parser():
    """The main parser for Cmon

    :return: the parser
    """
    from cmon.parsers.base import set_base_parser
    from cmon.parsers.create import set_new_project_parser
    from cmon.parsers.export import set_export_parser
    from cmon.parsers.flow import set_flow_parser
    from cmon.parsers.helper import _SHOW_ALL_ARGS, _chf
    ...
    set_pod_parser(
        sp.add_parser(
            ...
            description='Start an Executor. Cmon uses Executors process Documents',
    ...
    set_flow_parser(
        sp.add_parser(
            'flow',
            description='Start a Flow. Cmon uses Flows to streamline and distribute Executors',
            help='Start a Flow',
            formatter_class=_chf,
        )
    )
    ...
    set_export_parser(
        sp.add_parser(
            'export',
            help='Export Cmon API/Flow',
            description='Export Cmon API and Flow to JSONSchema, Kubernetes YAML, or SVG flowchart',
            formatter_class=_chf,
        )
    )
    ...
    set_new_project_parser(
        sp.add_parser(
            'new',
            help='Create a new Cmon project',
            description='Create a new Cmon project with a predefined template',
            formatter_class=_chf,
        )
    )
    ...
    get_auth_parser(
        sp.add_parser(
            'auth',
            description='Log in to Cmon AI with your GitHub/Google/Email account',
            formatter_class=_chf,
            help='Login to Cmon AI',
        )
    )
    ...
    get_jcloud_parser(
        sp.add_parser(
            'cloud',
            description='Manage Flows on Cmon Cloud',
            formatter_class=_chf,
            help='Manage Flows on Cmon Cloud',
        )
    )
    ...
    set_client_cli_parser(
        sp.add_parser(
            'client',
            description='Start a Python client that connects to a Cmon Gateway',
            formatter_class=_chf,
