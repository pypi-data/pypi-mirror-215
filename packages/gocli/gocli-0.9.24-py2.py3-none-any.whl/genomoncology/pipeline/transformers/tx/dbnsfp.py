...
def none_to_nan(x):  # pragma: no mccabe
    """as per kms-5025, the null items were not getting passed to solr.
    for example, if we sent a list [1,2,3,None,5] then solr would only
    ingest [1,2,3,5]. This was corrupting our data since the index in the list
    act as place holders for the genetic and variant information. We fixed this
    by replacing None with "NaN" since solr will allow "NaN" to be used as
     a place holder for float fields. We use "None"
     if it is a string field and -999999 if its an int field.

     Any fields that have all Nones, -999999 or Nan get purged from the dict.
     """
    keys_marked_for_removal = []
    values_to_purge = (None, 'None', -999999, "NaN")
    for k, v in x.items():
        if type(v) == list and k.endswith("__mstring"):
            if None in v:
                v = [val if val is not None else "None" for val in v]
                x[k] = v
                if all(val in values_to_purge for val in x[k]):
                    keys_marked_for_removal.append(k)
        if type(v) == list and (
                k.endswith("__mfloat")):
            v = [val if val is not None else "NaN" for val in v]
            x[k] = v
            if all(val in values_to_purge for val in x[k]):
                keys_marked_for_removal.append(k)
        if type(v) == list and (k.endswith("__mint")):
            v = [val if val is not None else -999999 for val in v]
            x[k] = v
            if all(val in values_to_purge for val in x[k]):
                keys_marked_for_removal.append(k)
    for key in keys_marked_for_removal:
        x.pop(key, None)
    return x
