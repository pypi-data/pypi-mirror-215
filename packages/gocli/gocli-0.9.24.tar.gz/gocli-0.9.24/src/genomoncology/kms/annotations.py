...
async def async_load_annotations(
    ...
    if is_custom:
        endpoint = sdk.annotations.postapiannotationsload_custom
    else:
        endpoint = sdk.annotations.load_annotations
...
def add_annotation_match(
    annotations_list: AnnotationMatchList,
...
def add_hgvs_annotation(
    ...
    if annotations_list:
        annotations = (
            obj_to_dict(fields, annotations_list.get_annotation(call)) or {}
        )
        if expected_bundle_version is not None:
...
"""
Used while parsing sv_hashes in the sv_match command.
alt (str) = component from parsed sv hash that looks like this:
'G]chr7:13819655]'
This function uses regex rules to determine
if a relative orientation is 'same' or 'reverse'
"""
