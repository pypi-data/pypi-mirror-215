...
import { Asset, ConnectorSnapshot } from '../PiecesSDK/common';
import { Format } from '../PiecesSDK/common';
...
import { setIsFetchFailed } from '../ui/piecesView';
import pollForNewSnippets, { stopPolling } from './poll_for_new_snippets';
...
const fetchSnapshot = async ({
    ...
    retry?: boolean;
}): Promise<ConnectorSnapshot | Error> => {
    const notifications: Notifications = Notifications.getInstance();

    let snapshot: ConnectorSnapshot;

    try {
        snapshot = await config.api.snapshot({
 ...
 * @returns
 */
export const processAssets = async ({ assets }: { assets: Asset[] }) => {
