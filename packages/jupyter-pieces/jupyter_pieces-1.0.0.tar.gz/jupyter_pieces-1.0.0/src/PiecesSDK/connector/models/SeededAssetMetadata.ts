...
export function SeededAssetMetadataFromJSONTyped(
    ...
    return {
        ...
        tags: !exists(json, 'tags')
            ? undefined
            : (json['tags'] as Array<any>).map(SeededAssetTagFromJSON),
        websites: !exists(json, 'websites')
            ? undefined
            : (json['websites'] as Array<any>).map(SeededAssetWebsiteFromJSON),
        sensitives: !exists(json, 'sensitives')
            ? undefined
            : (json['sensitives'] as Array<any>).map(
                  SeededAssetSensitiveFromJSON
              ),
        persons: !exists(json, 'persons')
            ? undefined
            : (json['persons'] as Array<any>).map(SeededPersonFromJSON),
    };
}
    ...
    return {
        ...
        tags:
            value.tags === undefined
                ? undefined
                : (value.tags as Array<any>).map(SeededAssetTagToJSON),
        websites:
            value.websites === undefined
                ? undefined
                : (value.websites as Array<any>).map(SeededAssetWebsiteToJSON),
        sensitives:
            value.sensitives === undefined
                ? undefined
                : (value.sensitives as Array<any>).map(
                      SeededAssetSensitiveToJSON
                  ),
        persons:
            value.persons === undefined
                ? undefined
                : (value.persons as Array<any>).map(SeededPersonToJSON),
    };
}
