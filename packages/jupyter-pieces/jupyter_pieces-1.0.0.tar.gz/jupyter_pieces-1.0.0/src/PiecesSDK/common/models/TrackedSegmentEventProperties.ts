...
export function TrackedSegmentEventPropertiesFromJSON(
    json: any
): TrackedSegmentEventProperties {
    return TrackedSegmentEventPropertiesFromJSONTyped(json, false);
}

export function TrackedSegmentEventPropertiesFromJSONTyped(
    json: any,
    ...
    return {
        ...
        asset: !exists(json, 'asset')
            ? undefined
            : TrackedAssetEventFromJSON(json['asset']),
        format: !exists(json, 'format')
            ? undefined
            : TrackedFormatEventFromJSON(json['format']),
        interaction: !exists(json, 'interaction')
            ? undefined
            : TrackedInteractionEventFromJSON(json['interaction']),
        keyboard: !exists(json, 'keyboard')
            ? undefined
            : TrackedKeyboardEventFromJSON(json['keyboard']),
        session: !exists(json, 'session')
            ? undefined
            : TrackedSessionEventFromJSON(json['session']),
        assets: !exists(json, 'assets')
            ? undefined
            : TrackedAssetsEventFromJSON(json['assets']),
    };
}

export function TrackedSegmentEventPropertiesToJSON(
    value?: TrackedSegmentEventProperties | null
): any {
    if (value === undefined) {
        return undefined;
    }
