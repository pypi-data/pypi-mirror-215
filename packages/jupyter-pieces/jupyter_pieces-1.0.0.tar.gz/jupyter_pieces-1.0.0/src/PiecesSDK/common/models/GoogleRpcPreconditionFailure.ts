...
export function GoogleRpcPreconditionFailureFromJSON(
    json: any
): GoogleRpcPreconditionFailure {
    return GoogleRpcPreconditionFailureFromJSONTyped(json, false);
}

export function GoogleRpcPreconditionFailureFromJSONTyped(
    json: any,
    ...
    return {
        violations: !exists(json, 'violations')
            ? undefined
            : (json['violations'] as Array<any>).map(
                  GoogleRpcPreconditionFailureViolationFromJSON
              ),
    };
}

export function GoogleRpcPreconditionFailureToJSON(
    value?: GoogleRpcPreconditionFailure | null
): any {
    if (value === undefined) {
        return undefined;
    }
    ...
    return {
        violations:
            value.violations === undefined
                ? undefined
                : (value.violations as Array<any>).map(
                      GoogleRpcPreconditionFailureViolationToJSON
                  ),
    };
}
