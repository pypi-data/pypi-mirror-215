...
export function AnalyticsTrackedSegmentEventPropertiesFromJSON(
    json: any
): AnalyticsTrackedSegmentEventProperties {
    return AnalyticsTrackedSegmentEventPropertiesFromJSONTyped(json, false);
}

export function AnalyticsTrackedSegmentEventPropertiesFromJSONTyped(
    json: any,
    ...
    return {
        ...
        user: !exists(json, 'user')
            ? undefined
            : AnalyticsTrackedUserProfileFromJSON(json['user']),
        asset: !exists(json, 'asset')
            ? undefined
            : AnalyticsTrackedAssetEventFromJSON(json['asset']),
        format: !exists(json, 'format')
            ? undefined
            : AnalyticsTrackedFormatEventFromJSON(json['format']),
        interaction: !exists(json, 'interaction')
            ? undefined
            : AnalyticsTrackedInteractionEventFromJSON(json['interaction']),
        keyboard: !exists(json, 'keyboard')
            ? undefined
            : AnalyticsTrackedKeyboardEventFromJSON(json['keyboard']),
        session: !exists(json, 'session')
            ? undefined
            : AnalyticsTrackedSessionEventFromJSON(json['session']),
        assets: !exists(json, 'assets')
            ? undefined
            : AnalyticsTrackedAssetsEventFromJSON(json['assets']),
    };
}

export function AnalyticsTrackedSegmentEventPropertiesToJSON(
    value?: AnalyticsTrackedSegmentEventProperties | null
): any {
    if (value === undefined) {
        return undefined;
    }
