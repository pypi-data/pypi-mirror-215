//@ts-nocheck
/* tslint:disable */
/* eslint-disable */
/**
 * Runtime Common Library
 * No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)
 *
 * The version of the OpenAPI document: 1.0
 *
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { exists, mapValues } from '../runtime';
import {
    AnalyticsIdentifiers,
    AnalyticsIdentifiersFromJSON,
    AnalyticsIdentifiersFromJSONTyped,
    AnalyticsIdentifiersToJSON,
    AnalyticsTrackedApplication,
    AnalyticsTrackedApplicationFromJSON,
    AnalyticsTrackedApplicationFromJSONTyped,
    AnalyticsTrackedApplicationToJSON,
    AnalyticsTrackedOS,
    AnalyticsTrackedOSFromJSON,
    AnalyticsTrackedOSFromJSONTyped,
    AnalyticsTrackedOSToJSON,
    SeededTrackedEvent,
    SeededTrackedEventFromJSON,
    SeededTrackedEventFromJSONTyped,
    SeededTrackedEventToJSON,
} from './';

/**
 * A common model for both the segment track endpoint and the analytics.
 * @export
 * @interface SeededAnalyticsTrackedGlobalEvent
 */
export interface SeededAnalyticsTrackedGlobalEvent {
    /**
     *
     * @type {AnalyticsTrackedOS}
     * @memberof SeededAnalyticsTrackedGlobalEvent
     */
    os?: AnalyticsTrackedOS;
    /**
     *
     * @type {AnalyticsTrackedApplication}
     * @memberof SeededAnalyticsTrackedGlobalEvent
     */
    application?: AnalyticsTrackedApplication;
    /**
     *
     * @type {AnalyticsIdentifiers}
     * @memberof SeededAnalyticsTrackedGlobalEvent
     */
    identifiers?: AnalyticsIdentifiers;
    /**
     *
     * @type {SeededTrackedEvent}
     * @memberof SeededAnalyticsTrackedGlobalEvent
     */
    event: SeededTrackedEvent;
    /**
     *
     * @type {string}
     * @memberof SeededAnalyticsTrackedGlobalEvent
     */
    timestamp: string;
}

export function SeededAnalyticsTrackedGlobalEventFromJSON(
    json: any
): SeededAnalyticsTrackedGlobalEvent {
    return SeededAnalyticsTrackedGlobalEventFromJSONTyped(json, false);
}

export function SeededAnalyticsTrackedGlobalEventFromJSONTyped(
    json: any,
    ignoreDiscriminator: boolean
): SeededAnalyticsTrackedGlobalEvent {
    if (json === undefined || json === null) {
        return json;
    }
    return {
        os: !exists(json, 'os')
            ? undefined
            : AnalyticsTrackedOSFromJSON(json['os']),
        application: !exists(json, 'application')
            ? undefined
            : AnalyticsTrackedApplicationFromJSON(json['application']),
        identifiers: !exists(json, 'identifiers')
            ? undefined
            : AnalyticsIdentifiersFromJSON(json['identifiers']),
        event: SeededTrackedEventFromJSON(json['event']),
        timestamp: json['timestamp'],
    };
}

export function SeededAnalyticsTrackedGlobalEventToJSON(
    value?: SeededAnalyticsTrackedGlobalEvent | null
): any {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
        return null;
    }
    return {
        os: AnalyticsTrackedOSToJSON(value.os),
        application: AnalyticsTrackedApplicationToJSON(value.application),
        identifiers: AnalyticsIdentifiersToJSON(value.identifiers),
        event: SeededTrackedEventToJSON(value.event),
        timestamp: value.timestamp,
    };
}
