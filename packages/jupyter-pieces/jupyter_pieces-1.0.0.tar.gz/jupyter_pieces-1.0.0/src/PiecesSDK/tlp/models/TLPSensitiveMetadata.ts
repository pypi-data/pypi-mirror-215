...
export function TLPSensitiveMetadataFromJSONTyped(
    json: any,
    ...
    return {
        match: !exists(json, 'match')
            ? undefined
            : TLPTextMatchFromJSON(json['match']),
        entropy: !exists(json, 'entropy') ? undefined : json['entropy'],
    };
}

export function TLPSensitiveMetadataToJSON(
    value?: TLPSensitiveMetadata | null
): any {
    if (value === undefined) {
        return undefined;
    }
