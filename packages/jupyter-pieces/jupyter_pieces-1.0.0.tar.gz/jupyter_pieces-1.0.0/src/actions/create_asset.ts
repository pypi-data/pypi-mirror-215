...
import { AssetSnapshotRequest } from '../PiecesSDK/core';
...
export default async function createAsset(
    ...
    try {
        ...
        const fetchAssetParams: AssetSnapshotRequest = {
            transferables: true,
            asset: id,
        };
        const asset = await config.assetApi.assetSnapshot(fetchAssetParams);
        storage.prependAsset({ asset: asset });
        return id;
    } catch (error: any) {
        if (retry) {
