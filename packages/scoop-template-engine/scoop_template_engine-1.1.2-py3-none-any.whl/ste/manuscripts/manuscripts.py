...
""".format(customPreambleFileNameWithoutExtension = self.CustomPreambleFileNameWithoutExtension(),
    ...
    def AuthorAMS(self, author):
        ...
        authorString = authorString + self.CommandWrapper(self.AddressesFromAuthor(author), "\\address[" + self.AuthorShortName(author) + "]")
        authorString = authorString + "\n"
        authorString = authorString + self.CommandWrapper(self.AuthorEMails(author), "\\email")
        authorString = authorString + "\n"
    ...
    def AuthorElsevier(self, author):
        ...
        authorString = authorString + "\n"
        authorString = authorString + self.CommandWrapper(self.AuthorEMails(author), "\\ead")
        authorString = authorString + "\n"
    ...
    def InstitutionETNA(self, institutionTag):
        emailString = ", ".join(self.EnsureList(self.CommandWrapper(self.AuthorEMailsIfFirstFromInstitutionTag(institutionTag), "\\texttt", ", ")))
        if emailString:
    ...
    def AuthorsWiley(self):
        ...
        authorString = authorString + "\n"
        authorString = authorString + self.CommandWrapper(". ".join((lambda self: [self.AuthorFullName(author) + ". " + self.CommandWrapper(", ".join(self.AuthorEMails(author)), "\\email") for author in self.CorrespondingAuthors()])(self)), "\\corres")
        return authorString
    ...
    # returns a string such as
    #   givenname familyname\orcidlink{0000-0000-0000-0000}\thanks{Institute, City, Country (\email{email}, \email{email})}
    # or
    #   givenname familyname\orcidlink{0000-0000-0000-0000}\footnotemark[1]\footnoteseparator\footnotemark[2]
    # or
    #   givenname familyname\orcidlink{0000-0000-0000-0000}\thanks{Institute, City, Country (\email{email}, \email{email})}\footnoteseparator\footnotemark[1]
    # .
