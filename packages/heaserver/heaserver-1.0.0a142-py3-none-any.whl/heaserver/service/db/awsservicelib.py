...
from heaserver.service.activity import S3ClientDesktopObjectActionLifecycle
...
async def create_object(request: web.Request, type_: S3Object | None = None, activity_cb: Optional[
    ...
    async with S3ClientDesktopObjectActionLifecycle(request=request,
                                              code='hea-create',
                                              description=f'Creating {_activity_object_display_name(bucket_id, folder_or_file_key)}',
                                              volume_id=volume_id,
                                              activity_cb=activity_cb) as (s3_client, _):
...
async def archive_object(request: Request,
    ...
    try:
        ...
        logger.exception(e)
        return response.status_bad_request(str(e))
    async with S3ClientDesktopObjectActionLifecycle(request,
                                              code='hea-archive',
...
async def unarchive_object(request: Request,
    ...
    try:
        async with S3ClientDesktopObjectActionLifecycle(request=request,
                                                ...
                                                volume_id=volume_id,
                                                activity_cb=activity_cb) as (s3_client, activity):
            try:
        ...
        return response.status_bad_request(str(exception))
    except:
        return response.status_internal_error(str(exception))
