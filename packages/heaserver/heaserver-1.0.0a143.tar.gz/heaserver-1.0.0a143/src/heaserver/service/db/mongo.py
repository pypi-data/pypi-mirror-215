...
class Mongo(Database):
    ...
    async def upsert(self, request: web.Request,
                     collection: str,
                     var_parts=None,
                     mongoattributes=None,
                     sub: str | None = None) -> UpdateResult | None:
        coll = self.get_collection(collection)
        try:
            mongo_expr_ = self.__mongo_expr(request,
                                            var_parts, mongoattributes,
                                            sub,
                                            set(PermissionGroup.PUTTER_PERMS.perms) | set(PermissionGroup.POSTER_PERMS.perms))
            return await coll.update_one(mongo_expr_, upsert=True)
        except bson.errors.InvalidId:
            pass
        return None
