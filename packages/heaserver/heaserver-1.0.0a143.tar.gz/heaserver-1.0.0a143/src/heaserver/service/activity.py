...
class DesktopObjectActionLifecycle(AbstractAsyncContextManager[DesktopObjectAction | None], ABC):
    ...
    async def __aenter__(self) -> DesktopObjectAction | None:
    ...
    async def __aexit__(self, exc_type: Type[BaseException] | None,
        ...
        if self.__activity_cb and self.__activity is not None:
            if exc_type is not None:
                ...
                if exc_type is None:
                    ...
                    self.__activity.status = Status.FAILED
            await publish_desktop_object(self.request.app, self.__activity)
