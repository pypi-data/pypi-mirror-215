...
class PreSkipTrans(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        output = tf.reshape(output, [batchsize, self.pt, self.skip, input_shape[1]])
        ...
        output = tf.reshape(output, [batchsize, self.pt, input_shape[1]])
...
class PostSkipTrans(tf.keras.layers.Layer):
	...
	#   the original batchsize
        ...
        output = tf.reshape(output, [batchsize, input_shape[1]])
...
class PreARTrans(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        #output = tf.reshape(output, [batchsize, self.hw])
...
#     if init.CNNFilters > 0 and init.CNNKernel > 0:
    ...
    C = Reshape((1,input_shape[1], input_shape[2]))(X) # (,,1)

# Apply a Conv2D that will transform it into data of dimensions (batchsize, time, 1, NumofFilters)
    C = Conv2D(filters=input_shape[2], kernel_size=(2),padding="same", kernel_initializer="glorot_uniform")(C)
    C = Dropout(0.2)(C)
...
#         C = X
    C = Reshape((input_shape[1], input_shape[2]))(C) # (,,1)
    # GRU
    # Apply a GRU layer (with activation set to 'relu' as per the paper) and take the returned states as result
    _, R = GRU(100, activation="relu", return_sequences = False, return_state = True)(C)
    R    = Dropout(0.2)(R)
...
#     else: # Adam
    opt  = Adam(0.001, beta_1=0.9, beta_2=0.999, epsilon=None, decay=0.0, amsgrad=False)

    # Compile using the previously defined metrics
    model.compile(optimizer = opt, loss = "mse", metrics=[rse, corr])
