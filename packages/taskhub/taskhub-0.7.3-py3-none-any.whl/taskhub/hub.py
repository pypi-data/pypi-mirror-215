        ...
        while True:
            try:
                ...
                for key, task in self.tasks.items():
                    status_count.setdefault(task.status, 0)
                    status_count[task.status] += 1

                logger.info(" ".join(f"{status}:{count}" for status, count in status_count.items()))
                ...
                logger.error("未知异常：{}".format(traceback.format_exc()))
